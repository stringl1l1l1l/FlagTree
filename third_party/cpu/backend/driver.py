...
# Locate the triton _C directory where TritonCPURuntime.so and sleef.so live
try:
...
class CPUUtils(object):
    ...
    def load_binary(self, name, kernel, shared_mem, device):
        with tempfile.NamedTemporaryFile(mode="wb", suffix=".so") as f:
            ...
            f.flush()
            import ctypes
            # Preload runtime dependencies with RTLD_GLOBAL so the kernel .so
            # can find libsleef and libTritonCPURuntime at dlopen time.
            for dep in ["libsleef.so", "libTritonCPURuntime.so"]:
                dep_path = next(
                    (str(d / dep) for d in library_dirs if (d / dep).exists()),
                    None,
                )
                if dep_path:
...
static void run_omp_kernels(uint32_t gridX, uint32_t gridY, uint32_t gridZ,
                            int num_threads, kernel_ptr_t kernel_ptr
...
static PyObject* launch(PyObject* self, PyObject* args) {{
  ...
  run_omp_kernels(gridX, gridY, gridZ, num_threads, kernel_ptr
                  {(', ' + ', '.join(f"ptr_info{i}.dev_ptr" if ty[0]=="*" else f"arg{i}" for i, ty in signature_without_constexprs.items())) if len(signature_without_constexprs) > 0 else ''});
