...
def _resolve_benchmark_driver(device_type: str | None):
...
def do_bench(fn, warmup=25, rep=100, grad_to_none=None, quantiles=None, return_mode="mean", device_type=None):
    ...
    benchmark_driver = _resolve_benchmark_driver(device_type)
    di = benchmark_driver.get_device_interface()

    fn()
    di.synchronize()

    cache = benchmark_driver.get_empty_cache_for_benchmark()
    ...
    start_event.record()
    for _ in range(5):
        benchmark_driver.clear_cache(cache)
        fn()
    end_event.record()
    di.synchronize()
    estimate_ms = start_event.elapsed_time(end_event) / 5
