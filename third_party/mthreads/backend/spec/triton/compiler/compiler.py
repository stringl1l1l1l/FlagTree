...
def parse(full_name, ext, context):
    ...
    if ext == "cubin" or ext == "hsaco" or ext == "mubin":
...
def _should_bypass_compilation_cache(target: GPUTarget) -> bool:
    # Replace-IR knobs override compiler stages outside Triton's cache hash.
    # Bypass cache hits so replacement file edits are always observed.
    if target.backend != "musa":
        return False
    return bool(knobs.musa.replace_llir or knobs.musa.replace_mubin)
...
def compile(src, target=None, options=None, _env_vars=None):
    ...
    bypass_cache = _should_bypass_compilation_cache(target)
    if not always_compile and metadata_path is not None and not bypass_cache:
        # cache hit!
    ...
    for ext, compile_ir in list(stages.items())[first_stage:]:
        ...
        if (not store_only_binary) or (ext in ("cubin", "hsaco", "mubin", "json")):
