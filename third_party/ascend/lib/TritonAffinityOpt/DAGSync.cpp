...
/**
 * Reworked alias analysis inplace of triton::SharedMemoryAliasAnalysis or
 * mlir::AliasAnalysis where both failed to establish aliases from function
 * arguments (i.e. global memory):
 * 1. triton::SharedMemoryAliasAnalysis - does not reach global memory
 * 2. mlir::AliasAnalysis - marks all function arguments as MayAlias or
 * PartialAlias; we may be able to get around by marking all arguments as
 * noalias, but it may affect other analyses...
 */
AliasInfo getAlias(triton::FuncOp funcOp, Graph::ValueMapRaw &valueMap) {
      ...
      if (auto *opNode = dyn_cast<OpNode>(output)) {
        for (auto result : opNode->op->getResults()) {
          if (valueIsPtrOrShapedPtr(result)) {
            worklist.push_back({result, node->value});
          }
        }
      } else if (auto *valueNode = dyn_cast<ValueNode>(output)) {
        worklist.push_back({valueNode->value, node->value});
      }
    }
  };
  ...
  funcOp.walk([&](Operation *op) {
    // If an op generates ptr from nowhere (e.g. tt.empty()), then we consider
    // it as a root
    bool generatesRootPtr =
        !llvm::any_of(op->getOperands(), valueIsPtrOrShapedPtr);
    if (generatesRootPtr) {
  ...
  while (!worklist.empty()) {
     ...
     * 2. aliasInfo[upstream] : steady reference without any insertion into the
     * densemap, will not cause resize, since upstream is in the map
...
bool mayAlias(AliasInfo &aliasInfo, Value valueA, Value valueB) {
  ...
  if (!valueIsPtrOrShapedPtr(valueA) || !valueIsPtrOrShapedPtr(valueB) ||
      aliasesA == nullptr || aliasesB == nullptr) {
    return false;
  }
...
// setOp, waitOp
std::pair<SyncBlockSetOp, SyncBlockWaitOp>
createBlockSyncBetween(OpBuilder builder, hivm::PIPE srcPipe,
                       hivm::PIPE dstPipe, SyncCandidate candidate, int flag) {
 ...
 *
 * Strangely, triton memory ops do not register their pointers to memory
 * effects, so we need some special cases as fallback here
 */
Value getMemoryAddress(Operation *op, MemoryEffects::EffectInstance effect) {
  ...
  return TypeSwitch<Operation *, Value>(op)
      .Case<triton::StoreOp, triton::LoadOp, triton::AtomicRMWOp,
            triton::AtomicCASOp>([](auto op) { return op.getPtr(); })
...
void addMemEffectsSync(triton::FuncOp funcOp, Graph *graph, OpBuilder &builder,
    ...
    for (auto &effect : effects) {
      ...
      for (auto &[prevNode, prevEffect] : memOps) {
        ...
        if ((isa<MemoryEffects::Write>(prevEffect.getEffect()) || isWrite) &&
          ...
          if (op->getParentOfType<LoopLikeOpInterface>()) { // need to insert
                                                            // backward sync
            backwardCandidates.push_back(SyncCandidate{
                dstCoreType, op, waitBefore, prevNode->op, setAfter});
          }
        }
      }
    }
   ...
   * Specially, for backward syncs, we also perform a kick-start for first wait
   * and clean-up to avoid affecting possible future syncs
   */
  auto addBlockSyncCommon = [&builder, &syncFlag,
                             &aliasInfo](SyncCandidate cand) {
    ...
    auto [forwardSet, forwardWait] = createBlockSyncBetween(
        builder, srcPipe, dstPipe, cand, syncFlag % MAX_FLAG_ID);
    ...
    if (loopAncestor) { // backward sync, need to kick-start and clean-up; only
                        // need to ensure the first and last one, because we
                        // ensure once a flag is consumed, it will be set by
                        // construction (wait-set in the same block)
      while (auto newAncestor =
                 loopAncestor->getParentOfType<LoopLikeOpInterface>()) {
      ...
      syncFlag++;
      auto backwardCandidate =
          SyncCandidate{!cand.srcCoreType, cand.waitCause, cand.waitBefore,
                        cand.setCause, cand.setAfter};
      auto [backwardSet, backwardWait] = createBlockSyncBetween(
          builder, srcPipe, dstPipe, backwardCandidate, syncFlag % MAX_FLAG_ID);

      backwardSet->setAttr("ssbuf.backward", builder.getUnitAttr());
      backwardWait->setAttr("ssbuf.backward", builder.getUnitAttr());
      backwardSet->setAttr("ssbuf.flagid",
                           builder.getUI32IntegerAttr(backendFlag));
      backwardWait->setAttr("ssbuf.flagid",
                            builder.getUI32IntegerAttr(backendFlag));

      builder.setInsertionPoint(loopAncestor);
      auto kickstart = createBlockSync<SyncBlockSetOp>(
          builder, toHivm(backwardCandidate.srcCoreType), srcPipe, dstPipe,
          syncFlag, backwardCandidate.setCause);

      builder.setInsertionPointAfter(loopAncestor);
      auto cleanup = createBlockSync<SyncBlockWaitOp>(
          builder, toHivm(!backwardCandidate.srcCoreType), srcPipe, dstPipe,
          syncFlag, backwardCandidate.waitCause);
    }
  ...
  /**
   * Sorts the candidates by (setAfter, waitBefore), and then removes redundant
   * sync candidates that fully contains another
   */
  auto selectCandidatesToInsertSync =
      [&setAfterDominate, &waitBeforePostDominate,
       &addBlockSyncCommon](llvm::SmallVectorImpl<SyncCandidate> &candidates) {
        llvm::sort(
            candidates, [&](const SyncCandidate &a, const SyncCandidate &b) {
