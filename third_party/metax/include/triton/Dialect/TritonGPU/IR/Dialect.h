...
#ifdef USE_MACA
llvm::SmallVector<int> inline getMmaThreadShape(bool needTrans = false) {
  if (!needTrans) {
    return {16, 16, 4};
  } else {
    return {4, 4, 16};
...
llvm::SmallVector<unsigned> inline getMACAMmaThreadNum(bool colMajor = false) {
  if (!colMajor) {
    return {4, 16}; // {threadM, threadN}
  } else {
    return {16, 4};
  }
}

inline unsigned getLdsTransVec(unsigned elemBytes) {
  if (elemBytes == 2) {
    return 4;
  } else if (elemBytes == 1) {
    return 8;
...
inline unsigned getLdsTransVec(Type elemTy) {
  if (elemTy.isF16() || elemTy.isBF16()) {
    return 4;
  } else if (elemTy.isSignlessInteger(8)) {
    return 8;
  } else {
    return 1;
  }
...
int getNumRepK(int K, int elemsPerK);
