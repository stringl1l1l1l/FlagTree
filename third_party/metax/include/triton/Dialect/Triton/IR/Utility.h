...
template <typename T>
llvm::SmallVector<T> getSubMultiDimIndexImpl(T linearIndex,
...
template <typename T>
llvm::SmallVector<T> getSubMultiDimIndex(T linearIndex, llvm::ArrayRef<T> shape,
                                         llvm::ArrayRef<unsigned> order) {
  size_t rank = shape.size();
  assert(rank == order.size());
  auto reordered = applyPermutation(shape, order);
  auto reorderedMultiDim = getSubMultiDimIndexImpl<T>(linearIndex, reordered);
