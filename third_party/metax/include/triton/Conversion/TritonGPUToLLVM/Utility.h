...
#ifdef USE_MACA
inline SmallVector<unsigned> emitSubOffsetForBlockedAndDotLayout(
    const Attribute &DistributedLayout, RankedTensorType type,
    ArrayRef<int64_t> ctaIdx, ArrayRef<int64_t> elemIdx) {
  auto srcLayout = type.getEncoding();
  assert(isa<triton::gpu::BlockedEncodingAttr>(srcLayout) ||
         isa<triton::gpu::DotOperandEncodingAttr>(srcLayout));
  llvm::SmallVector<unsigned> sizePerThread;
  if (isa<triton::gpu::BlockedEncodingAttr>(srcLayout)) {
    auto blockSrcLayout = dyn_cast<triton::gpu::BlockedEncodingAttr>(srcLayout);
    sizePerThread = llvm::to_vector(blockSrcLayout.getSizePerThread());
  } else if (isa<mlir::triton::gpu::DotOperandEncodingAttr>(srcLayout)) {
    auto dotLayout =
        dyn_cast<mlir::triton::gpu::DotOperandEncodingAttr>(srcLayout);
    auto mmaLayout =
        dyn_cast<mlir::triton::gpu::MACAMmaEncodingAttr>(dotLayout.getParent());
    auto elemsMnk = mmaLayout.getElementsMNK();
    auto opIdx = dotLayout.getOpIdx();
    if (opIdx == 0) {
      sizePerThread = {elemsMnk[0], elemsMnk[2]};
    } else {
      sizePerThread = {elemsMnk[2], elemsMnk[1]};
    }
  } else {
    auto dotLinearEnc = dyn_cast<triton::gpu::LinearEncodingAttr>(srcLayout);
    if (!dotLinearEnc) {
      assert(false);
    }
    sizePerThread = llvm::to_vector(dotLinearEnc.getSizePerThread());
  }
...
inline SmallVector<unsigned> emitSubOffsetForMmaLayoutMACA(
    const mlir::triton::gpu::MACAMmaEncodingAttr &mmaLayout,
    RankedTensorType type, ArrayRef<int64_t> ctaIdx,
    ArrayRef<int64_t> elemIdx) {
  auto shapePerCTA = mlir::triton::gpu::getShapePerCTATile(
      type); // elemM*16*warp, elemN*16*warp
  auto srcEncodingLinear = triton::gpu::toLinearEncoding(type);
  ...
  // {1, elemsN, elemsN * elemsM, elemsN * elemsM * 4, elemsN * elemsM * 4 *
  // RepN} tn + tm * elemsN + depth * elemsN * elemsM + repn * elemsN * elemsM *
  // 4 + repm * elemsN * elemsM * 4 * RepN
      ...
      for (int d = 0; d < 4; d++) {
        if (!mmaLayout.getColMajor()) {
          linear = elemNVec[j] + elemMVec[j] * elemsN + d * elemsN * elemsM +
                   repNVec[i] * elemsN * elemsM * 4 +
                   repMVec[i] * elemsN * elemsM * 4 * RepN;
        } else {
          linear = elemNVec[j] + d * elemsN + repNVec[i] * 4 * elemsN +
                   elemMVec[j] * 4 * elemsN * RepN +
                   repMVec[i] * 4 * elemsN * RepN * elemsM;
        }
...
inline SmallVector<unsigned> emitSubOffsetForLayout(Attribute layout,
  ...
  if (auto blockedLayout =
          dyn_cast<mlir::triton::gpu::BlockedEncodingAttr>(srcLayout))
    return emitSubOffsetForBlockedAndDotLayout(blockedLayout, type, ctaIdx,
                                               elemIdx);
  if (auto mmaLayout =
          dyn_cast<mlir::triton::gpu::MACAMmaEncodingAttr>(srcLayout))
    return emitSubOffsetForMmaLayoutMACA(mmaLayout, type, ctaIdx, elemIdx);
  if (auto dotLayout =
          dyn_cast<mlir::triton::gpu::DotOperandEncodingAttr>(srcLayout))
    return emitSubOffsetForBlockedAndDotLayout(dotLayout, type, ctaIdx,
                                               elemIdx);
