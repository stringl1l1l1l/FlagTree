...
static SmallVector<Value> computeWarpLevelHistogram(
    ...
    const TargetInfo &targetInfo, triton::HistogramOp op) {
  ...
  for (int i = 0; i < numElementsPerThreads; ++i) {
    ...
    for (int j = 0; j < numBits; ++j) {
      ...
      Value bit = targetInfo.ballotIntrinsic(rewriter, loc,
                                             int_ty(numThreadPerWarp), cmp, op);
    ...
    if (maskValues.size() > 0) {
      inputMaskBit = targetInfo.ballotIntrinsic(
          rewriter, loc, int_ty(numThreadPerWarp), maskValues[i], op);
    }
...
public:
  ...
  matchAndRewrite(triton::HistogramOp op, OpAdaptor adaptor,
    ...
    SmallVector<Value> warpLevelHistogram = computeWarpLevelHistogram(
        ...
        threadId, rewriter, targetInfo, op);
...
void mlir::triton::METAX::populateHistogramOpToLLVMPatterns(
