...
static void warpScan(SmallVector<SmallVector<Value>> &srcValues,
                     ConversionPatternRewriter &rewriter,
                     const TargetInfo &targetInfo, ScanLoweringHelper &helper,
                     Value laneIdAxis, triton::ScanOp op) {
  ...
  for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
    ...
    for (unsigned i = 1; i <= scanDim / 2; i <<= 1) {
      ...
      for (unsigned j = 0; j < acc.size(); ++j) {
        shfl[j] = targetInfo.shuffleUpIntrinsic(rewriter, loc, acc[j],
                                                i * threadStride, op);
      }
...
static void AddPartialReduce(SmallVector<SmallVector<Value>> &srcValues,
  ...
  for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
    ...
    for (unsigned i = 0; i < helper.getNumOperands(); ++i) {
      auto elem = targetInfo.shuffleUpIntrinsic(rewriter, loc, temp[i],
                                                threadStride, op);
      lastElement[i] =
...
static void AddPartialReduceOneWarp(SmallVector<SmallVector<Value>> &srcValues,
  ...
  for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
    ...
    if (scanDim > 1) {
      for (unsigned i = 0; i < helper.getNumOperands(); ++i) {
        lastElement[i] = targetInfo.shuffleUpIntrinsic(
            rewriter, loc, srcValues[srcIndex][i], threadStride, op);
        lastElement[i] =
            b.select(maskFirstLane, accumulator[i], lastElement[i]);
        if (numScanBlocks > 1)
          // Update accumulator with the value from the last lane.
          accumulator[i] = targetInfo.shuffleIdxIntrinsic(
              rewriter, loc, srcValues[srcIndex][i], laneIdLast, op);
      }
  ...
  for (int i = 0; i < srcValues.size(); ++i) {
    ...
    for (unsigned j = 0; j < op.getNumOperands(); ++j) {
      for (unsigned k = iWarpSize / 2; k >= 1; k = k / 2) {
        srcValues[revIndex][j] = targetInfo.shuffleXorIntrinsic(
            rewriter, loc, srcValues[revIndex][j], k, op);
      }
...
ScanOpConversion::emitFastScan(triton::ScanOp op, triton::ScanOpAdaptor adaptor,
  ...
  // elements.
  warpScan(srcValues, rewriter, targetInfo, helper, laneIdAxis, op);
  ...
  if (axisNumWarps > 1) {
    ...
    AddPartialReduce(srcValues, rewriter, targetInfo, helper, smemBases,
                     smemTypes, warpIdAxis, laneIdAxis, flatIdParallel, op);
...
void mlir::triton::METAX::populateScanOpToLLVMPatterns(
