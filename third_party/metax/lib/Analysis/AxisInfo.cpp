...
#ifdef USE_MACA
class ExtractTensorOpAxisInfoVisitor final
    : public AxisInfoVisitorImpl<triton::gpu::ExtractTensorOp> {
public:
  using AxisInfoVisitorImpl<triton::gpu::ExtractTensorOp>::AxisInfoVisitorImpl;
  AxisInfo
  getAxisInfo(triton::gpu::ExtractTensorOp op,
              ArrayRef<const dataflow::Lattice<AxisInfo> *> operands) override {
    AxisInfo opInfo = operands[0]->getValue();
    ...
    auto subSizes =
        dyn_cast<RankedTensorType>(op.getResult().getType()).getShape();
    ...
    auto const_val = opInfo.getConstantValue();
    AxisInfo::DimVectorT contiguity;
    AxisInfo::DimVectorT divisibility;
    AxisInfo::DimVectorT constancy;
    for (int d = 0; d < opInfo.getRank(); ++d) {
      long divisible_val = 1;
      if (opContiguity[d] >= srcShape[d]) {
        contiguity.push_back(subSizes[d]);
        divisible_val = gcd(opDivisibility[d], subSizes[d]);
      } else {
        contiguity.push_back(1);
      }
      if (opConstancy[d] >= srcShape[d]) {
        constancy.push_back(subSizes[d]);
        divisible_val = opDivisibility[d];
      } else {
        constancy.push_back(1);
      }
      divisibility.push_back(divisible_val);
    }
    return AxisInfo(contiguity, divisibility, constancy, const_val);
  }
};
...
public:
  using AxisInfoVisitorImpl<triton::gpu::InsertTensorOp>::AxisInfoVisitorImpl;
  AxisInfo
  getAxisInfo(triton::gpu::InsertTensorOp op,
              ArrayRef<const dataflow::Lattice<AxisInfo> *> operands) override {
    AxisInfo dst_opInfo = operands[0]->getValue();
    AxisInfo src_opInfo = operands[1]->getValue();
    auto dst = dyn_cast<RankedTensorType>(op.getInserted().getType());
    if (!dst)
      return AxisInfo();
    auto dstShape = dst.getShape();
    auto subSizes = dstShape;
    AxisInfo::DimVectorT dstContiguity = dst_opInfo.getContiguity();
    AxisInfo::DimVectorT dstDivisibility = dst_opInfo.getDivisibility();
    AxisInfo::DimVectorT dstConstancy = dst_opInfo.getConstancy();
    auto dst_const_val = dst_opInfo.getConstantValue();
    AxisInfo::DimVectorT srcContiguity = src_opInfo.getContiguity();
    AxisInfo::DimVectorT srcDivisibility = src_opInfo.getDivisibility();
    AxisInfo::DimVectorT srcConstancy = src_opInfo.getConstancy();
    auto src_const_val = src_opInfo.getConstantValue();
    AxisInfo::DimVectorT contiguity;
    AxisInfo::DimVectorT divisibility;
    AxisInfo::DimVectorT constancy;
    std::optional<long> const_val;
    assert(dst_opInfo.getRank() == 2 && "only support 2d tensor insert");
    assert(src_opInfo.getRank() == 2 && "only support 2d tensor insert");
    for (int d = 0; d < dst_opInfo.getRank(); ++d) {
      if (dstContiguity[d] >= dstShape[d] && srcContiguity[d] >= subSizes[d]) {
        contiguity.push_back(srcContiguity[d]);
      } else {
        contiguity.push_back(1);
      }
      divisibility.push_back(gcd(dstDivisibility[d], srcDivisibility[d]));
      if (dstConstancy[d] >= dstShape[d] && srcConstancy[d] >= subSizes[d]) {
        if (dst_const_val && src_const_val && dst_const_val == src_const_val) {
          constancy.push_back(dstConstancy[d]);
          const_val = dst_const_val;
        } else {
          constancy.push_back(srcConstancy[d]);
        }
      } else {
        constancy.push_back(1);
      }
...
class SwizzleTensorOpAxisInfoVisitor final
    : public AxisInfoVisitorImpl<triton::gpu::SwizzleTensorOp> {
public:
  using AxisInfoVisitorImpl<triton::gpu::SwizzleTensorOp>::AxisInfoVisitorImpl;
  AxisInfo
  getAxisInfo(triton::gpu::SwizzleTensorOp op,
              ArrayRef<const dataflow::Lattice<AxisInfo> *> operands) override {
    AxisInfo opInfo = operands[0]->getValue();
    ...
    auto minVec = std::min(op.getInVec(), op.getOutVec());
    ...
    AxisInfo::DimVectorT opConstancy = opInfo.getConstancy();
    auto const_val = opInfo.getConstantValue();
    AxisInfo::DimVectorT contiguity = opContiguity;
    AxisInfo::DimVectorT divisibility = opDivisibility;
    AxisInfo::DimVectorT constancy = opConstancy;
    divisibility[inOrder[0]] = 1;
    if (maxPhase == 1)
      return AxisInfo(contiguity, divisibility, constancy, const_val);
    if (opContiguity[inOrder[0]] >= srcShape[inOrder[0]]) {
      contiguity[inOrder[0]] = minVec;
      divisibility[inOrder[0]] = gcd(opDivisibility[inOrder[0]], minVec);
    } else if (opContiguity[inOrder[0]] == minVec) {
      contiguity[inOrder[0]] = minVec;
    } else {
      contiguity[inOrder[0]] = 1;
    }
    if ((opConstancy[inOrder[0]] >= srcShape[inOrder[0]]) ||
        (opConstancy[inOrder[0]] == minVec)) {
      divisibility[inOrder[0]] = opDivisibility[inOrder[0]];
    } else {
      constancy[inOrder[0]] = 1;
    }
    return AxisInfo(contiguity, divisibility, constancy, const_val);
  }
};
