...
void MembarAnalysis::update(Operation *op, BlockInfo *blockInfo,
                            ...
                            OpBuilder *builder) {
#ifdef USE_MACA
  bool disableOptMACA = std::getenv("TRITON_DISABLE_MACA_OPT_MMA");
  if (!disableOptMACA) {
    if (isa<triton::gpu::LocalLoadOp>(op)) {
      auto localLoadOp = dyn_cast<triton::gpu::LocalLoadOp>(op);
      bool isIntrinsic = localLoadOp.getIntrinsic();
      if (isIntrinsic)
        return;
    }
    if (isa<triton::gpu::LocalStoreOp>(op)) {
      auto localStoreOp = dyn_cast<triton::gpu::LocalStoreOp>(op);
      bool isIntrinsic = localStoreOp.getIntrinsic();
      if (isIntrinsic)
        return;
    }
    if (isa<triton::gpu::ConvertLayoutOp>(op)) {
      auto cvtOp = dyn_cast<triton::gpu::ConvertLayoutOp>(op);
      bool isIntrinsic = cvtOp.getIntrinsic();
      if (isIntrinsic)
        return;
    }
    if (isa<triton::gpu::AsyncCopyGlobalToLocalOp>(op)) {
      auto asyncCopyOp = dyn_cast<triton::gpu::AsyncCopyGlobalToLocalOp>(op);
      bool isIntrinsic = asyncCopyOp.getIntrinsic();
      if (isIntrinsic)
        return;
    }
  }
#endif
#ifdef USE_MACA
  if (isa<gpu::BarrierOp, triton::gpu::BarrierOp, triton::gpu::LocalBarrierOp,
          triton::gpu::BarrierSharedOp>(op)) {
#else
...
#ifdef USE_MACA
      !isa<gpu::BarrierOp, triton::gpu::BarrierOp, triton::gpu::LocalBarrierOp,
           triton::gpu::BarrierSharedOp>(op->getNextNode())) {
#else
