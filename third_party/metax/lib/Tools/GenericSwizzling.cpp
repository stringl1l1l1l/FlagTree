...
#ifdef USE_MACA
  // compared with cuda, maca gpu own 64 threads per warp, only 32 of them need
  // to be considered for bank conflict, therefore we drop_back one more dim.
  auto tileSrc = to_vector(ArrayRef(laneSrc).drop_back(log2Vec + 1));
  auto tileDst = to_vector(ArrayRef(laneDst).drop_back(log2Vec + 1));
#else
...
optimalSwizzling(const LinearLayout &src, const LinearLayout &dst,
  ...
  if (tiles.empty()) {
    ...
    llvm::dbgs() << "optimalSwizzlingLdSt forceNoVec: " << forceNoVec << "\n";
#ifdef USE_MACA
    llvm::dbgs() << "optimalSwizzlingLdSt forceNoVec1: " << forceNoVec << "\n";
    auto smem = optimalSwizzlingLdSt(src, dst, bitwidth, forceNoVec);
#else
