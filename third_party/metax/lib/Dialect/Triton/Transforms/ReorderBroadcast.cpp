...
// divf(a, splat(b)) => mulf(a, splat(rcpf(b)))
// reduce MTE instructions by convert multiple divf instructions to a common
// rcpf instruction
struct ExtractCommonRcpFromDivPattern
    : public OpTraitRewritePattern<OpTrait::Elementwise> {

  ExtractCommonRcpFromDivPattern(mlir::MLIRContext *context)
      : OpTraitRewritePattern(context) {}
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    // ------------------ match ------------------------
    auto loc = op->getLoc();
    auto divOp = llvm::dyn_cast<mlir::arith::DivFOp>(op);
    if (!divOp) {
      return mlir::failure();
    }
    ...
    // rcpf only support float type
    RankedTensorType ty = dyn_cast<RankedTensorType>(rhs.getType());
    if (!ty) {
      return mlir::failure();
    }
    if (!ty.getElementType().isF16() && !ty.getElementType().isF32()) {
      return mlir::failure();
    }
    ...
    Value rcpfResult;
    DenseElementsAttr constAttr;
    if (auto splatOp = llvm::dyn_cast<triton::SplatOp>(rhsOp)) {
      auto divend = splatOp.getSrc();
      auto rcpfOp =
          rewriter.create<triton::RcpfOp>(loc, divend.getType(), divend);
      rcpfResult = rcpfOp.getResult();
    } else if (matchPattern(rhsOp, m_Constant(&constAttr)) &&
               constAttr.isSplat()) {
      auto value = constAttr.getSplatValue<Attribute>();
      Value divend;
      // #ifdef USE_MACA
      //         divend = rewriter.create<arith::ConstantOp>(
      //             loc, constAttr.getElementType(), value);
      // #else
      divend = arith::ConstantOp::materialize(rewriter, value,
                                              constAttr.getElementType(), loc);
      // #endif
      auto rcpfOp =
          rewriter.create<triton::RcpfOp>(loc, divend.getType(), divend);
      rcpfResult = rcpfOp.getResult();
    ...
    // 2. splat it
    auto newSplat =
        rewriter.create<triton::SplatOp>(loc, rhs.getType(), rcpfResult);
    auto newSplatResult = newSplat.getResult();
    // 3. create mulf
    auto lhs = divOp.getLhs();
    auto mulOp = rewriter.create<mlir::arith::MulFOp>(loc, rhs.getType(), lhs,
                                                      newSplatResult);
    // 4. replace origin op
...
#ifdef USE_MACA
    // divf(a, splat(b)) => mulf(a, splat(rcpf(b)))
    if (!std::getenv("TRITON_DISABLE_EXTRACT_COMMON_RCP_FROM_DIV_PASS")) {
      patterns.add<ExtractCommonRcpFromDivPattern>(context);
    }
#endif
