...
LogicalResult Fp4ToFpScaledOp::verify() {
  ...
  for (int i = 0; i < rank; ++i) {
    if (i == axis) {
      ...
      if (inputShape[i] % 2 != 0)
        return emitError() << "axis " << axis
                           << " input dimension must be divisible by 2, got "
                           << inputShape[i];
      if (scaleShape[i] != inputShape[i])
        return emitError()
               << "axis " << axis
               << " scale dimension must match input dimension (input="
...
// -- ElementwiseInlineIntrinsicOp --
void ElementwiseInlineIntrinsicOp::getEffects(
    SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>
        &effects) {
  if (getPure())
    return;
