...
LinearLayout macaDotToLinearLayout(ArrayRef<int64_t> shape,
                                   DotOperandEncodingAttr dot) {
  int rank = shape.size();
  ...
  unsigned verMajor = mma.getVersionMajor();
  unsigned verMinor = mma.getVersionMinor();
  bool isATrans = mma.getIsATrans();
  bool isBTrans = mma.getIsBTrans();
  bool colMajor = mma.getColMajor();
  // TODO: support verMajor > 2
  assert(verMajor == 2 && "ONLY support versionMajor==2 right now");
  assert(!isATrans && !isBTrans && "LDS trans is not supported yet");

  auto elemsMNK = mma.getElementsMNK();
  int tM = elemsMNK[0];
  int tN = elemsMNK[1];
  int tK = elemsMNK[2];
  auto mmaThreadNum = getMmaThreadShape(false); // (16,16,4)
  int mThread = mmaThreadNum[0];
  int nThread = mmaThreadNum[1];
  int kThread = mmaThreadNum[2];
  ...
  if (isA) {
    ctaLayout *=
        LinearLayout::identity1D(tK, S("register"), outDimNames[kDim]) *
        LinearLayout::identity1D(tM, S("register"), outDimNames[mnDim]) *
        LinearLayout::identity1D(mThread, S("lane"), outDimNames[mnDim]) *
        LinearLayout::identity1D(kThread, S("lane"), outDimNames[kDim]);
  } else {
    ctaLayout *=
        LinearLayout::identity1D(tK, S("register"), outDimNames[kDim]) *
        LinearLayout::identity1D(tN, S("register"), outDimNames[mnDim]) *
        LinearLayout::identity1D(nThread, S("lane"), outDimNames[mnDim]) *
        LinearLayout::identity1D(kThread, S("lane"), outDimNames[kDim]);
  }
  ...
  auto rep = mma.getRepForOperand(shape, 0, 0, dot.getOpIdx());
  int repK = rep[kDim];
  if (isA) {
    int repM = rep[mnDim];
    ctaLayout *=
        LinearLayout::identity1D(repK, S("register"), outDimNames[kDim]) *
        LinearLayout::identity1D(repM, S("register"), outDimNames[mnDim]);
  } else {
    int repN = rep[mnDim];
    ctaLayout *=
        LinearLayout::identity1D(repK, S("register"), outDimNames[kDim]) *
        LinearLayout::identity1D(repN, S("register"), outDimNames[mnDim]);
  }
...
LinearLayout
MACAMmaEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const {
  auto ctx = getContext();
  int rank = shape.size();
  assert(rank == getRank());
  ...
  int tM = elemsMNK[0];
  int tN = elemsMNK[1];
  unsigned verMajor = getVersionMajor();
  unsigned verMinor = getVersionMinor();
  bool isATrans = getIsATrans();
  bool isBTrans = getIsBTrans();
  bool colMajor = getColMajor();

  // TODO: support verMajor > 2
  assert(verMajor == 2 && "ONLY support versionMajor==2 right now");
  assert(!isATrans && !isBTrans && "LDS trans is not supported yet");

  // rowmajor: (/*mThread=*/4, /*nThread=*/16)
  auto mmaThreadNum = getMACAMmaThreadNum(colMajor);
  // output C always has 4 elements (regs) every mma instruction
  const int numCPerInst = 4;
  if (colMajor) {
    tN *= numCPerInst;
  } else {
    tM *= numCPerInst;
  ...
  int mDim = 0;
  int nDim = 1;
  int64_t M = shape[mDim];
  int64_t N = shape[nDim];
  int mThread = mmaThreadNum[mDim];
  int nThread = mmaThreadNum[nDim];

  auto warpsPerCTA = getWarpsPerCTA(); // default order: (M, N)
  auto outDimNames = standardOutDimNames(ctx, rank);
  ...
  ctaLayout *= identityStandardND(S("lane"), mmaThreadNum, threadOrder)
                   .transposeOuts(llvm::to_vector(ctaLayout.getOutDimNames()));
  //   rowmajor:
  //         = LinearLayout::identity1D(nThread, S("lane"), outDimNames[nDim]) *
  //           LinearLayout::identity1D(mThread, S("lane"), outDimNames[mDim])
  auto warpOrder = getMatrixOrder(rank, /*rowMajor*/ true);
  ctaLayout *= identityStandardND(S("warp"), warpsPerCTA, warpOrder)
                   .transposeOuts(llvm::to_vector(ctaLayout.getOutDimNames()));
  //= LinearLayout::identity1D(warpsPerCTA[0], S("warp"), outDimNames[mDim]) *
  //  LinearLayout::identity1D(warpsPerCTA[1], S("warp"), outDimNames[nDim])

  int repM = std::max<int>(M / (warpsPerCTA[0] * mThread * tM), 1);
  int repN = std::max<int>(N / (warpsPerCTA[1] * nThread * tN), 1);
  ctaLayout *=
...
DotOperandEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const {
  ...
  } else if (auto macammaLayout = mlir::dyn_cast<MACAMmaEncodingAttr>(parent)) {
    return macaDotToLinearLayout(shape, *this);
  } else {
