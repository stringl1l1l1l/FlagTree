...
LogicalResult LocalLoadOp::verify() {
  ...
  if (getSrc().getType().getShape() != getSrc().getType().getShape()) {
    return this->emitOpError("source shape [")
           << getSrc().getType().getShape() << "] must match ["
           << "destination shape " << getSrc().getType().getShape() << "]";
  }
  return success();
}
