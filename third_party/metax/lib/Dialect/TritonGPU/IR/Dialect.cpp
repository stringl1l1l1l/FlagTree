...
#ifdef USE_MACA
int getNumRepM(int M, int wpt) {
  int mmaThreadM = getMmaThreadShape()[0];
  return std::max<int>(M / (wpt * mmaThreadM), 1);
}

int getNumRepM(int M, int wpt, int elemsPerM) {
  int mmaThreadM = getMmaThreadShape()[0];
  return std::max<int>(M / (wpt * mmaThreadM * elemsPerM), 1);
}

int getNumRepN(int N, int wpt) {
  int mmaThreadN = getMmaThreadShape()[1];
  return std::max<int>(N / (wpt * mmaThreadN), 1);
}

int getNumRepN(int N, int wpt, int elemsPerN) {
  int mmaThreadN = getMmaThreadShape()[1];
  return std::max<int>(N / (wpt * mmaThreadN * elemsPerN), 1);
}

int getNumRepK_(int K, int elemsPerK) {
  int mmaThreadK = getMmaThreadShape()[2];
  return std::max<int>(K / (mmaThreadK * elemsPerK), 1);
}

int getNumRepM(int M, ArrayRef<unsigned int> wpt) {
  return getNumRepM(M, wpt[0]);
}

int getNumRepM(int M, ArrayRef<unsigned int> wpt, int elemsPerM) {
  return getNumRepM(M, wpt[0], elemsPerM);
}

int getNumRepN(int N, ArrayRef<unsigned int> wpt) {
  return getNumRepN(N, wpt[1]);
}

int getNumRepN(int N, ArrayRef<unsigned int> wpt, int elemsPerN) {
  return getNumRepN(N, wpt[1], elemsPerN);
}

int getNumRepK(int K, int elemsPerK) { return getNumRepK_(K, elemsPerK); }
...
Attribute MACAMmaEncodingAttr::parse(AsmParser &parser, Type type) {
  ...
  return parser.getChecked<MACAMmaEncodingAttr>(
      parser.getContext(), versionMajor, versionMinor, warpsPerCTA, elementsMNK,
...
void MACAMmaEncodingAttr::print(AsmPrinter &printer) const {
  printer << "<{"
          ...
          << ", isATrans = " << getIsATrans()
          << ", isBTrans = " << getIsBTrans() << ", elementsStride = ["
          << getElementsStride() << "]";
  maybePrintCTALayout(getContext(), printer, getCTALayout(),
                      /*rank=*/getWarpsPerCTA().size());
  printer << "}>";
...
SmallVector<int64_t>
MACAMmaEncodingAttr::getRepForOperand(ArrayRef<int64_t> shape, int bitwidth,
                                      int kWidth, int opIdx) const {
  auto rank = shape.size();
  auto warpsPerCTA = to_vector(getWarpsPerCTA());
  auto kDim = opIdx == 0 ? rank - 1 : rank - 2;
  auto MNDim = opIdx == 0 ? rank - 2 : rank - 1;

  auto elementsMNK = getElementsMNK();
  int K = shape[kDim];
  int repK = getNumRepK(K, elementsMNK[2]);
  if (opIdx == 0) {
    int repM =
        getNumRepM(shape[MNDim], getWarpsPerCTA()[MNDim], elementsMNK[0]);
    return {repM, repK};
  } else {
    int repN =
        getNumRepN(shape[MNDim], getWarpsPerCTA()[MNDim], elementsMNK[1]);
    return {repK, repN};
  }
...
// getElemsPerThreadOrTrans
// getThreadShape
#ifdef USE_MACA
SmallVector<int> MACAMmaEncodingAttr::getThreadShape(bool needTrans) const {
  return getMmaThreadShape(needTrans);
}

SmallVector<unsigned> MACAMmaEncodingAttr::getElemsPerThreadOrTrans(
    DotOperandEncodingAttr &encoding) const {
  bool isA = encoding.getOpIdx() == 0;
  bool enableLdsTrans = isA ? getIsATrans() : getIsBTrans();
  auto elemsMNK = getElementsMNK();
  SmallVector<unsigned> elemsPerThread;
  unsigned ldsTransVec = isA ? getElementsStride()[0] : getElementsStride()[1];
  if (isA) {
    if (enableLdsTrans) {
      assert((elemsMNK[2] >= ldsTransVec) && (elemsMNK[2] % ldsTransVec == 0));
      elemsPerThread = {elemsMNK[0] * ldsTransVec, elemsMNK[2] / ldsTransVec};
    } else {
      elemsPerThread = {elemsMNK[0], elemsMNK[2]};
    }
    // return {m, k}
  } else {
    if (enableLdsTrans) {
      assert((elemsMNK[2] >= ldsTransVec) && (elemsMNK[2] % ldsTransVec == 0));
      elemsPerThread = {elemsMNK[2] / ldsTransVec, elemsMNK[1] * ldsTransVec};
    } else {
      elemsPerThread = {elemsMNK[2], elemsMNK[1]};
    }
    // return {k, n}
  }
...
SwizzledSharedEncodingAttr MACAMmaEncodingAttr::composeSharedLayoutForOperand(
    DotOperandEncodingAttr dotOpEnc, CTAEncodingAttr CTALayout,
    ArrayRef<int64_t> operandShape, ArrayRef<unsigned> sharedOrder,
    unsigned typeWidthInBit, bool needTrans) const {
  int opIdx = dotOpEnc.getOpIdx();
  // number of rows per phase
  int perPhase = 128 / (operandShape[sharedOrder[0]] * (typeWidthInBit / 8));
  perPhase = std::max<int>(perPhase, 1);

  if (std::getenv("TRITON_DISABLE_SWIZZLE") != nullptr) {
    return SwizzledSharedEncodingAttr::get(getContext(), 1, 1, 1, sharedOrder,
                                           CTALayout);
  }

  int eltByte = typeWidthInBit / 8;
  auto elemsMnk = getElementsMNK();
  int tm = elemsMnk[0];
  int tn = elemsMnk[1];
  int tk = elemsMnk[2];
  ...
  if (opIdx == 0) {
    auto elemsPerThread = getElemsPerThreadOrTrans(dotOpEnc);
    tm = elemsPerThread[0];
    tk = elemsPerThread[1];
    auto mmaThreadShape = getThreadShape(getIsATrans());
    int shapeM = !needTrans ? operandShape[0] : operandShape[1];
    int shapeK = !needTrans ? operandShape[1] : operandShape[0];

    bool shape_judge = (shapeM >= (mmaThreadShape[0] * tm)) &&
                       (shapeK >= (mmaThreadShape[2] * tk)) &&
                       (shapeM % (mmaThreadShape[0] * tm) == 0) &&
                       (shapeK % (mmaThreadShape[2] * tk) == 0);
    assert(shape_judge && "tn and tk not meet conditon");
    bool AT = (!needTrans && sharedOrder[0] == 1) ||
              (needTrans && sharedOrder[0] == 0);
    bool AN = (!needTrans && sharedOrder[0] == 0) ||
              (needTrans && sharedOrder[0] == 1);

    if (AT) {
      // There is no need lds trans for AT.
      assert(!getIsATrans());
      perPhase = std::max(tm, perPhase);
      int mul = std::max(perPhase / tm, 1);
      int vec = std::max(128 / (16 * eltByte), tk);
      int maxPhase = std::max(128 / (vec * eltByte) / mul, 8 / mul);
      maxPhase = std::min(maxPhase, shapeK / vec); // prevent overstep the
                                                   // bound;
      if (maxPhase == 1)
        vec = tk;
    ...
    if (AN) {
      perPhase = std::max(tk, perPhase);
      int mul = std::max(perPhase / tk, 1);
      int vec = mmaThreadShape[0] * tm;
      int maxPhase = std::max(128 / (vec * eltByte) / mul, 1);
      maxPhase = std::min(maxPhase, shapeM / vec); // prevent overstep the
                                                   // bound;
      if (maxPhase == 1)
        vec = tm;
      return SwizzledSharedEncodingAttr::get(getContext(), vec, perPhase,
  ...
  if (opIdx == 1) {
    ...
    tk = elemsPerThread[0];
    tn = elemsPerThread[1];
    auto mmaThreadShape = getThreadShape(getIsBTrans());
    int shapeK = !needTrans ? operandShape[0] : operandShape[1];
    int shapeN = !needTrans ? operandShape[1] : operandShape[0];

    bool shape_judge = (shapeK >= (mmaThreadShape[2] * tk)) &&
                       (shapeN >= (mmaThreadShape[1] * tn)) &&
                       (shapeK % (mmaThreadShape[2] * tk) == 0) &&
                       (shapeN % (mmaThreadShape[1] * tn) == 0);
    assert(shape_judge && "tm and tk not meet conditon");
    bool BT = (!needTrans && sharedOrder[0] == 1) ||
              (needTrans && sharedOrder[0] == 0);
    bool BN = (!needTrans && sharedOrder[0] == 0) ||
              (needTrans && sharedOrder[0] == 1);

    if (BN) {
      // There is no need lds trans for BN.
      assert(!getIsBTrans());
      perPhase = std::max(tn, perPhase);
      int mul = std::max(perPhase / tn, 1);
      int vec = std::max(128 / (16 * eltByte), tk);
      int maxPhase = std::max(128 / (vec * eltByte) / mul, 8 / mul);
      maxPhase = std::min(maxPhase, shapeK / vec); // prevent overstep the
                                                   // bound;
    ...
    if (BT) {
      perPhase = std::max(tk, perPhase);
      int mul = std::max(perPhase / tk, 1);
      int vec = mmaThreadShape[1] * tn;
      int maxPhase = std::max(128 / (vec * eltByte) / mul, 1);
      maxPhase = std::min(maxPhase, shapeN / vec); // prevent overstep the
                                                   // bound;
      if (maxPhase == 1)
        vec = tn;
      return SwizzledSharedEncodingAttr::get(getContext(), vec, perPhase,
...
mlir::LogicalResult mlir::triton::gpu::SwizzleTensorOp::inferReturnTypes(
  ...
  if (!srcTensorTy)
    return mlir::failure();
  // swizzle op only support int or index and non-pointer type.
  if ((!srcTensorTy.getElementType().isIntOrIndex()) ||
      isa<triton::PointerType>(srcTy))
    return mlir::failure();
...
calSubCtaNumAndSizePerThread(RankedTensorType srcTy,
                             ArrayRef<int64_t> linearCtaIdx,
                             ArrayRef<int64_t> linearElemIdx) {
  Attribute srcEncoding = srcTy.getEncoding();
  SmallVector<unsigned> retNumCtas, retSizePerThread;

  auto srcEncodingLinear = triton::gpu::toLinearEncoding(srcTy);
  auto sizePerThread = srcEncodingLinear.getSizePerThread();
  auto shapePerCTA = getShapePerCTATile(srcTy);

  SmallVector<unsigned> numReplica;
  for (int i = 0; i < shapePerCTA.size(); i++) {
    numReplica.push_back(ceil<unsigned>(srcTy.getShape()[i], shapePerCTA[i]));
  }

  SmallVector<unsigned> order;
  int rank;
  if (isa<mlir::triton::gpu::MACAMmaEncodingAttr>(srcEncoding)) {
    auto mmaLayout =
        dyn_cast<mlir::triton::gpu::MACAMmaEncodingAttr>(srcEncoding);
    sizePerThread = SmallVector<unsigned>{mmaLayout.getElementsMNK()[0],
                                          mmaLayout.getElementsMNK()[1]};
    ...
    rank = 2;
  } else if (isa<mlir::triton::gpu::BlockedEncodingAttr>(srcEncoding)) {
    order = mlir::triton::gpu::getOrder(srcTy);
    rank = order.size();
  } else if (isa<mlir::triton::gpu::DotOperandEncodingAttr>(srcEncoding)) {
    auto dotLayout =
        dyn_cast<mlir::triton::gpu::DotOperandEncodingAttr>(srcEncoding);
    ...
    llvm::report_fatal_error(
        "Unsupported layout for ExtractTensorOp Or InsertTensorOp");
    return std::make_pair(retNumCtas, retSizePerThread);
  }

  // unique ctaId & elemId of each dim
  SmallVector<SetVector<unsigned>> ctaIdSet, elemIdSet;
  ctaIdSet.resize(rank);
  elemIdSet.resize(rank);
  for (int i = 0; i < linearCtaIdx.size(); i++) {
    auto multiCtaId = mlir::triton::getSubMultiDimIndex<unsigned>(
        linearCtaIdx[i], numReplica, order);
    for (int j = 0; j < rank; j++) {
      ctaIdSet[j].insert(multiCtaId[j]);
    }
  }
  for (int i = 0; i < linearElemIdx.size(); i++) {
    auto multiElemId = mlir::triton::getSubMultiDimIndex<unsigned>(
        linearElemIdx[i], sizePerThread, order);
    for (int j = 0; j < rank; j++) {
      elemIdSet[j].insert(multiElemId[j]);
    }
  }

  for (int i = 0; i < rank; i++) {
    retNumCtas.push_back(ctaIdSet[i].size());
    retSizePerThread.push_back(elemIdSet[i].size());
  }

  return std::make_pair(retNumCtas, retSizePerThread);
}

mlir::LogicalResult mlir::triton::gpu::ExtractTensorOp::inferReturnTypes(
    MLIRContext *context, std::optional<Location> location, ValueRange operands,
    DictionaryAttr attributes, OpaqueProperties properties, RegionRange regions,
    SmallVectorImpl<Type> &inferredReturnTypes) {
  auto src = operands[0];
  ...
  if (!srcTy)
    return mlir::failure();
  Attribute srcEncoding = srcTy.getEncoding();
  auto srcEltTy = srcTy.getElementType();

  auto linearCtaIdx = properties.as<Properties *>()->ctaIdx.asArrayRef();
  auto linearElemIdx = properties.as<Properties *>()->elemIdx.asArrayRef();

  auto srcEncodingLinear = triton::gpu::toLinearEncoding(srcTy);
  auto sizePerThread = srcEncodingLinear.getSizePerThread();
  auto threadsPerWarp = srcEncodingLinear.getThreadsPerWarp();
  auto warpsPerCTA = srcEncodingLinear.getWarpsPerCTA();
  auto shapePerCTA = getShapePerCTATile(srcTy);

  if (!isa<mlir::triton::gpu::MACAMmaEncodingAttr>(srcEncoding) &&
      !isa<mlir::triton::gpu::DotOperandEncodingAttr>(srcEncoding) &&
      !isa<mlir::triton::gpu::BlockedEncodingAttr>(srcEncoding))
    return mlir::failure();

  auto res = calSubCtaNumAndSizePerThread(srcTy, linearCtaIdx, linearElemIdx);
  auto retNumCtas = res.first;
  auto retSizePerThread = res.second;
  int rank = retNumCtas.size();
  ...
  if (isa<mlir::triton::gpu::MACAMmaEncodingAttr>(srcEncoding)) {
    ...
    SmallVector<unsigned> newElemMNK = {retSizePerThread[0],
                                        retSizePerThread[1],
                                        mmaLayout.getElementsMNK()[2]};
    if (mmaLayout.getColMajor()) {
      retSizePerThread[1] *= 4;
    } else {
      retSizePerThread[0] *= 4;
    }
    for (int i = 0; i < rank; i++)
      retShape[i] = retSizePerThread[i] * threadsPerWarp[i] * warpsPerCTA[i] *
                    retNumCtas[i];
    auto retEncoding = mlir::triton::gpu::MACAMmaEncodingAttr::get(
        srcTy.getContext(), mmaLayout.getVersionMajor(),
        mmaLayout.getVersionMinor(), warpsPerCTA, newElemMNK,
        mmaLayout.getColMajor(), mmaLayout.getIsATrans(),
        mmaLayout.getIsBTrans(), mmaLayout.getElementsStride());
    inferredReturnTypes.push_back(
        RankedTensorType::get(retShape, srcEltTy, retEncoding));

  } else if (isa<mlir::triton::gpu::BlockedEncodingAttr>(srcEncoding)) {
    for (int i = 0; i < rank; i++)
      retShape[i] = retSizePerThread[i] * threadsPerWarp[i] * warpsPerCTA[i] *
                    retNumCtas[i];
    auto retEncoding = mlir::triton::gpu::BlockedEncodingAttr::get(
        srcTy.getContext(), retSizePerThread, threadsPerWarp, warpsPerCTA,
        mlir::triton::gpu::getOrder(srcTy),
        mlir::triton::gpu::getCTALayout(srcEncoding));
    inferredReturnTypes.push_back(
        RankedTensorType::get(retShape, srcEltTy, retEncoding));

  } else if (isa<mlir::triton::gpu::DotOperandEncodingAttr>(srcEncoding)) {
    // Only Support extract in either M or N dim, not both!
    auto dotLayout =
        dyn_cast<mlir::triton::gpu::DotOperandEncodingAttr>(srcEncoding);
    auto mmaLayout =
        dyn_cast<mlir::triton::gpu::MACAMmaEncodingAttr>(dotLayout.getParent());
    auto warpsPerCTAMMA = mmaLayout.getWarpsPerCTA();
    auto opIdx = dotLayout.getOpIdx();
    SmallVector<unsigned> shapePerCTA, newElemMNK;

    if (opIdx == 0) {
      shapePerCTA = {16 * warpsPerCTA[0] * retSizePerThread[0],
                     4 * retSizePerThread[1]};
      newElemMNK = {retSizePerThread[0], mmaLayout.getElementsMNK()[1],
                    retSizePerThread[1]};
    } else if (opIdx == 1) {
      shapePerCTA = {4 * retSizePerThread[0],
                     16 * warpsPerCTA[1] * retSizePerThread[1]};
      newElemMNK = {mmaLayout.getElementsMNK()[0], retSizePerThread[1],
                    retSizePerThread[0]};
    }

    for (int i = 0; i < rank; i++)
      retShape[i] = shapePerCTA[i] * retNumCtas[i];

    auto parentEncoding = mlir::triton::gpu::MACAMmaEncodingAttr::get(
        srcTy.getContext(), mmaLayout.getVersionMajor(),
        mmaLayout.getVersionMinor(), warpsPerCTAMMA, newElemMNK,
        mmaLayout.getColMajor(), mmaLayout.getIsATrans(),
        mmaLayout.getIsBTrans(), mmaLayout.getElementsStride());
    auto retEncoding = mlir::triton::gpu::DotOperandEncodingAttr::get(
        srcTy.getContext(), opIdx, parentEncoding, srcEltTy);
    inferredReturnTypes.push_back(
        RankedTensorType::get(retShape, srcEltTy, retEncoding));
  } else {
    return mlir::failure();
...
mlir::LogicalResult mlir::triton::gpu::InsertTensorOp::inferReturnTypes(
    MLIRContext *context, std::optional<Location> location, ValueRange operands,
    DictionaryAttr attributes, OpaqueProperties properties, RegionRange regions,
    SmallVectorImpl<Type> &inferredReturnTypes) {
  auto inserted = operands[0];
  auto insert = operands[1];
  auto insertedTy = dyn_cast<RankedTensorType>(inserted.getType());
  if (!insertedTy)
    return mlir::failure();
  auto insertTy = cast<RankedTensorType>(insert.getType());
  if (!insertTy)
    return mlir::failure();
  auto retShape = insertedTy.getShape().vec();
  auto insertShape = insertTy.getShape().vec();
  Attribute srcEncoding = insertedTy.getEncoding();

  auto insertedTyllEnc = triton::gpu::toLinearEncoding(insertedTy);
  auto warpsPerCTA = insertedTyllEnc.getWarpsPerCTA();
  auto threadsPerWarp = insertedTyllEnc.getThreadsPerWarp();

  // check if infered shape from CTAidx and elemIdx == insert shape
  if (!isa<mlir::triton::gpu::MACAMmaEncodingAttr>(srcEncoding) &&
      !isa<mlir::triton::gpu::DotOperandEncodingAttr>(srcEncoding) &&
      !isa<mlir::triton::gpu::BlockedEncodingAttr>(srcEncoding))
    return mlir::failure();
  ...
  auto linearElemIdx = properties.as<Properties *>()->elemIdx.asArrayRef();
  auto res =
      calSubCtaNumAndSizePerThread(insertedTy, linearCtaIdx, linearElemIdx);
  auto retNumCtas = res.first;
  auto retSizePerThread = res.second;
  int rank = retNumCtas.size();
  SmallVector<int64_t> subShape(rank, 1);
  ...
  if (isa<mlir::triton::gpu::MACAMmaEncodingAttr>(srcEncoding)) {
    ...
    for (int i = 0; i < rank; i++)
      subShape[i] = retSizePerThread[i] * threadsPerWarp[i] * warpsPerCTA[i] *
                    retNumCtas[i];
  } else if (isa<mlir::triton::gpu::BlockedEncodingAttr>(srcEncoding)) {
    for (int i = 0; i < rank; i++)
      subShape[i] = retSizePerThread[i] * threadsPerWarp[i] * warpsPerCTA[i] *
    ...
    for (int i = 0; i < rank; i++)
      subShape[i] = shapePerCTA[i] * retNumCtas[i];
  }
  for (int k = 0; k < subShape.size(); ++k) {
    if (subShape[k] != insertShape[k]) {
      llvm::report_fatal_error("failed to infer layout for InsertTensorOp");
      return mlir::failure();
    }
  }
  auto insertedEltTy = insertedTy.getElementType();
  inferredReturnTypes.push_back(
      RankedTensorType::get(retShape, insertedEltTy, srcEncoding));
  return mlir::success();
}
#endif
//===----------------------------------------------------------------------===//
...
#ifdef USE_MACA
  if (auto parentAttr = mlir::dyn_cast<MACAMmaEncodingAttr>(parent)) {
    return success();
  }
#endif
...
struct TritonGPUInferLayoutInterface
  ...
  LogicalResult inferFp4ToFpScaledOpEncoding(ArrayRef<int64_t> shape, int axis,
                                             Attribute inEnc, Attribute &outEnc,
                                             bool fwdInference,
                                             std::optional<Location> loc,
    ...
    if (getOrder(cast<DistributedEncodingTrait>(inEnc), shape)[axis] == 0) {
      // Dot operand: double kWidth if kDim == axis.
      if (auto dotEnc = mlir::dyn_cast<DotOperandEncodingAttr>(inEnc)) {
        auto mmaEnc = mlir::dyn_cast<MACAMmaEncodingAttr>(dotEnc.getParent());
        if (mmaEnc) {
          auto elemMNK = mmaEnc.getElementsMNK();
          SmallVector<unsigned> newElemMNK = {elemMNK[0], elemMNK[1],
                                              2 * elemMNK[2]};
          auto newMmaEnc = MACAMmaEncodingAttr::get(
              ctx, mmaEnc.getVersionMajor(), mmaEnc.getVersionMinor(),
              mmaEnc.getWarpsPerCTA(), newElemMNK, mmaEnc.getColMajor(),
              mmaEnc.getIsATrans(), mmaEnc.getIsBTrans(),
              mmaEnc.getElementsStride());
          outEnc = DotOperandEncodingAttr::get(ctx, dotEnc.getOpIdx(),
                                               dotEnc.getParent(), outputTy);
          return success();
        } else {
          return failure();
        }
