...
#else
      ors, [&b](Value x, Value y) { return b.or_(x, y); });
#endif
  auto xorPart =
...
lowerLdStShared(Location loc, MLIRContext *ctx, LinearLayout cvt,
  ...
  auto emitLdSt = [&](RewriterBase &rewriter, Location loc,
    ...
    if (isStore) {
      ...
      Value valsVec;
#ifdef USE_MACA
      // For int8 LDS, replace load with intrinsic 'llvm.mxc.lds' to reduce
      // shift instructions.
      ...
      // (TODO): Reason for check 'length > 1': MC3-9454
      if (std::getenv("TRITON_DISABLE_V16I8_LDS") == nullptr &&
          elemTy.isSignlessInteger(8) && isLoadOp && length > 1) {
        valsVec = targetInfo.loadDSharedIntrinsic(
            rewriter, loc, shmemAddr, std::nullopt, vecTy,
            /*pred=*/b.true_val(), localLoadOp);
        SmallVector<Value> results;
        for (int i = 0; i < length; i++) {
          results.push_back(b.extract_element(valsVec, b.i32_val(i)));
        }
        return results;
      } else {
