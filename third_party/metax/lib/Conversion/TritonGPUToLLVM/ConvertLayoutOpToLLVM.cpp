...
#ifdef USE_MACA
    const char *swizzle_dump_path = std::getenv("TRITON_SWIZZLE_DUMP_PATH");
    if (swizzle_dump_path != nullptr) {
      dumpSwizzleAccessCsv(std::string(swizzle_dump_path), totalStoreCvt,
                           totalLoadCvt, &srcLayout, &dstLayout);
    }
#endif
