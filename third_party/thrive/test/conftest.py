...
def pytest_addoption(parser):
    parser.addoption("--device", action="store", default="thrive")
...
_SKIP_FILES = ["test_tensor_descriptor", "test_subprocess"]


def pytest_collection_modifyitems(config, items):
    skip_marker = pytest.mark.skip(reason="[Thrive] not supported on thrive")
    for item in items:
        if any(name in item.nodeid for name in _SKIP_FILES):
