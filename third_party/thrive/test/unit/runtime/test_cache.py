...
def test_kernel_default_arg(device):
    ...
    kernel[(1, )](x)
    assert x.cpu() == torch.ones_like(x.cpu())
...
def test_jit_warmup_cache(device) -> None:
    ...
    args = [
        torch.randn(32, dtype=torch.float32).to(device),
        torch.randn(32, dtype=torch.float32).to(device),
        torch.randn(32, dtype=torch.float32).to(device),
        32,
    ]
...
def test_jit_debug(device) -> None:
    ...
    device = getattr(torch, device).current_device()
    tmp = torch.tensor([1], dtype=torch.int32).to(device)
...
@pytest.mark.skip(reason="[Thrive] accessing Tensors by index is temporarily not supported on thrive")
def test_function_arguments(device):
    ...
    y = torch.zeros((5, ), dtype=torch.int32).to(device)
...
def test_async_compile(device, fresh_triton_cache):
    ...
    with (
        ...
        a = torch.empty((16, 16)).to(device)
        b = torch.empty((16, 16), dtype=torch.int32).to(device)
        kernel.warmup(a, 0, grid=(1, ))
        kernel.warmup(a, 1, grid=(1, ))
        kernel.warmup(b, 0, grid=(1, ))
        kernel.warmup(b, 1, grid=(1, ))
