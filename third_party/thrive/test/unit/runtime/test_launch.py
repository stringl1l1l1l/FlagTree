...
def test_memory_leak(device) -> None:
    ...
    tracemalloc.start()
    try:
        inp = torch.randn(10, ).to(device)
        out = torch.randn(10, ).to(device)
...
def test_launch_with_options(options) -> None:
    if "extern_libs" in options:
        ...
        # Thrive by default
        libdir = current_dir.parent.parent.parent.parent / 'thrive/lib'
        options["extern_libs"] = {"libdevice": str(libdir / 'vpu_libdevice.bc')}
...
def test_pre_run_hooks(device):
    ...
    n_elements = 4
    a = torch.ones(n_elements, dtype=torch.int32).to(device)
    add_kernel[(1, )](a, n_elements)
    assert torch.all(a.cpu() == 2)
