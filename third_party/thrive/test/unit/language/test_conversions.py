...
def launch_type_convert_triton(src, src_dtype, dst_dtype, device, rounding=None, BLOCK_SIZE=1024):

    dst = torch.empty(src.shape, dtype=matching_int(dst_dtype)).to(device)
...
def launch_exhaustive_populate(dst_dtype, offset, numel, force_odd, output_bits, max_repr, device, BLOCK_SIZE=1024):

    assert(numel % BLOCK_SIZE == 0)
    dst = torch.empty((numel,), dtype=matching_int(dst_dtype)).to(device)
...
def launch_downcast_emulated(src, src_dtype, dst_dtype, rounding, exponent_bits, mantissa_bits, exponent_bias, device, BLOCK_SIZE=1024):
...
def launch_upcast_emulated(src, exponent_bits, mantissa_bits, exponent_bias, device, BLOCK_SIZE=1024):

    dst = torch.empty(src.shape, dtype=torch.int32).to(device)
...
def downcast_test(src_dtype, dst_dtype, rounding, exponent_bits, mantissa_bits, exponent_bias, max_repr, offset, numel, device):

    src = launch_exhaustive_populate(src_dtype, offset * numel, numel, False, src_dtype.primitive_bitwidth, max_repr, device)
...
@pytest.mark.parametrize("src_dtype, dst_dtype", [
    ...
    pytest.param('float8e4b15', 'float16', marks=pytest.mark.skip(reason="[Thrive] not supported on thrive")),
    pytest.param('float8e4b15', 'float32', marks=pytest.mark.skip(reason="[Thrive] not supported on thrive")),

    ('float8e4nv', 'float16'),
    ('float8e4nv', 'bfloat16'),
    ('float8e4nv', 'float32'),

    pytest.param('float8e4b8', 'float32', marks=pytest.mark.skip(reason="[Thrive] not supported on thrive")),
    pytest.param('float8e4b8', 'bfloat16', marks=pytest.mark.skip(reason="[Thrive] not supported on thrive")),
    pytest.param('float8e4b8', 'float16', marks=pytest.mark.skip(reason="[Thrive] not supported on thrive")),

    pytest.param('float8e5b16', 'float32', marks=pytest.mark.skip(reason="[Thrive] not supported on thrive")),
    pytest.param('float8e5b16', 'float16', marks=pytest.mark.skip(reason="[Thrive] not supported on thrive")),
])
...
@pytest.mark.parametrize("src_dtype, dst_dtype, rounding, max_repr", [
    ...
    pytest.param('float32', 'float16', 'rtz', 0x477fe000, marks=pytest.mark.skip(reason="[Thrive] known issues")),
    ('float32', 'bfloat16', 'rtne', 0x7f7f0000),
    pytest.param('float32', 'bfloat16', 'rtz', 0x7f7f0000, marks=pytest.mark.skip(reason="[Thrive] known issues")),
    ('float32', 'float8e5', 'rtne', 0x47600000),
    pytest.param('float32', 'float8e5', 'rtz', 0x47600000, marks=pytest.mark.skip(reason="[Thrive] known issues")),
    ('float32', 'float8e4nv', 'rtne', 0x43e00000),
    pytest.param('float32', 'float8e4b8', 'rtne', 0x43700000, marks=pytest.mark.skip(reason="[Thrive] not supported on thrive")),
    pytest.param('float32', 'float8e5b16', 'rtne', 0x47600000, marks=pytest.mark.skip(reason="[Thrive] not supported on thrive")),
    ...
    pytest.param('bfloat16', 'float8e5b16', 'rtne', 0x4760, marks=pytest.mark.skip(reason="[Thrive] not supported on thrive")),
    pytest.param('bfloat16', 'float8e4b8', 'rtne', 0x4370, marks=pytest.mark.skip(reason="[Thrive] not supported on thrive")),

    pytest.param('float16', 'float8e5b16', 'rtne', 0x7b00, marks=pytest.mark.skip(reason="[Thrive] not supported on thrive")),
    pytest.param('float16', 'float8e4b8', 'rtne', 0x5b80, marks=pytest.mark.skip(reason="[Thrive] not supported on thrive")),
])
def test_typeconvert_downcast(src_dtype, dst_dtype, rounding, max_repr, device):
    ...
    if device == 'thrive':
        # To exhaustively populate all the values of fp32 would require 4096 batches,
        # but there is no such exhaustive populate here
        numel = 1024  # < 1024 * 1024
    else:
        numel = 2**24

    for i in range(128):
        downcast_test(getattr(tl, src_dtype), getattr(tl, dst_dtype), rounding, *stuff, max_repr, i, numel=numel, device=device)
...
def test_typeconvert_downcast_clamping(src_dtype, dst_dtype, mode, device, rounding="rtne"):
    ...
    BLOCK_SIZE = 1024
    shape = (BLOCK_SIZE * 2,)
    src = torch.full(shape, test_value, dtype=torch_src_dtype).to(device)
    dst = torch.empty(shape, dtype=torch_dst_dtype).to(device)
    ...
    if mode == 'nan':
        assert(torch.all(torch.isnan(dst.cpu())))
    else:
        torch.testing.assert_close(dst.cpu(), torch.full_like(dst.cpu(), expected_result))
