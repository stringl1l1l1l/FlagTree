...
def test_list_of_functions():
    ...
    # CHECK: call @{{.*}}list_of_functions_constexpr{{.*}}cJITFunction({{.*}}:anchor){{.*}}cJITFunction({{.*}}:forward)
...
def test_aggregate_with_constexpr():
    ...
    # CHECK: tt.call @"{{.*}}test_frontend.add_rhs_constexpr__{{.*}}test_frontend.AggregateWithConstexpr<i32S4S, constexpr_type[42]>
    agg = AggregateWithConstexpr.create(tl.arange(0, 4))
    add_rhs_constexpr(agg)

    # CHECK: tt.func private @"{{.*}}test_frontend.add_rhs_constexpr__{{.*}}test_frontend.AggregateWithConstexpr<i32S4S, constexpr_type[42]>
...
def test_aggregate_with_tuple():
    ...
    # CHECK: tt.call @"{{.*}}test_frontend.pass_tuple_aggregate__{{.*}}test_frontend.AggregateWithTuple<Ti32S4ST>__"
    agg = AggregateWithTuple.create(tl.arange(0, 4))
    pass_tuple_aggregate(agg)
    # CHECK: tt.func private @"{{.*}}test_frontend.pass_tuple_aggregate__{{.*}}test_frontend.AggregateWithTuple<Ti32S4ST>__"
