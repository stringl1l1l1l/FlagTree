...
def test_randint(size, seed, device, dtype, const_seed):
    ...
    # triton result
    x = torch.empty(size, dtype=torch_dtype).to(device)
    N = x.numel()
...
def test_rand(size, seed, dtype, device, const_seed):
    ...
    # triton result
    x = torch.empty(size, dtype=torch.float32).to(device)
    N = x.numel()
    grid = (triton.cdiv(N, BLOCK.value), )
    if const_seed:
    ...
    assert all((x.cpu() >= 0) & (x.cpu() <= 1))
    assert scipy.stats.kstest(x.cpu().tolist(), "uniform", args=(0, 1)).statistic < 0.01
...
def test_seed_is_int(device):
    ...
    x = torch.empty(1, dtype=torch.float32).to(device)
    with pytest.raises(triton.compiler.errors.CompilationError):
        seed0 = torch.zeros(1, dtype=torch.int32).to(device)
        kernel[(1, )](x, seed0)
    with pytest.raises(triton.compiler.errors.CompilationError):
        seed1 = 2.3
        kernel[(1, )](x, seed1)
...
def test_randn(size, seed, dtype, device, const_seed):
    ...
    if const_seed:
        ...
        kernel[grid](x, N, seed, dtype=getattr(tl, dtype))
    assert abs(x.cpu().mean()) < 1e-2
    assert abs(x.cpu().std() - 1) < 1e-2
...
def test_rand_limits(dtype, device):
    ...
    min_max_int = torch.tensor(
        [torch.iinfo(torch_dtype).min, torch.iinfo(torch_dtype).max],
        dtype=torch_dtype,
    ).to(device)
    output = torch.empty(2, dtype=torch.float32).to(device)
    kernel[(1, )](min_max_int, output, 2)

    assert output.cpu()[0] == output.cpu()[1]
    assert 1.0 - torch.finfo(torch.float32).eps <= output.cpu()[0].item() < 1.0
