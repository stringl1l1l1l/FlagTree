...
class TestMXFP4Tensor(MXBaseTest):
    ...
    def test_padding(self, device):
        tensor_pad = MXFP4Tensor(torch.tensor([4]).to(device))
        pad_packed = tensor_pad.to_packed_tensor(dim=0)
        torch.testing.assert_close(tensor_pad.data,
                                   tensor_pad.unpack_packed_tensor(pad_packed, dim=0, original_shape=(1, )).cpu())

    def test_zero_values(self, device):
        test_values = torch.tensor([0.0, -0.0]).to(device)
        tensor = MXFP4Tensor(test_values)
        expected_encodings = torch.tensor([0b0000, 0b1000], dtype=torch.uint8).to(device)
    ...
    def test_out_of_range_values(self, device):
        ...
        tensor = MXFP4Tensor(test_values)
        expected_values = torch.tensor([6.0, -6.0, 6.0, -6.0]).to(device)
        torch.testing.assert_close(tensor.to(torch.float32), expected_values)

    def test_subnormal_numbers(self, device):
        test_values = torch.tensor([0.1, 0.2, 0.3, 0.4]).to(device)
        tensor = MXFP4Tensor(test_values)
        expected_values = torch.tensor([0.0, 0.0, 0.5, 0.5]).to(device)
        torch.testing.assert_close(tensor.to(torch.float32), expected_values)

    def test_rounding_edge_cases(self, device):
        test_values = torch.tensor([0.75, 1.25, 1.75, 2.5, 3.5, 5.0]).to(device)
        expected_values = torch.tensor([1.0, 1.0, 2.0, 2.0, 4.0, 4.0]).to(device)
        tensor = MXFP4Tensor(test_values)
        torch.testing.assert_close(tensor.to(torch.float32), expected_values)

    def test_negative_values(self, device):
        test_values = torch.tensor([-0.5, -1.0, -1.5, -2.0, -3.0, -4.0, -6.0]).to(device)
        tensor = MXFP4Tensor(test_values)
        torch.testing.assert_close(tensor.to(torch.float32), test_values)

    def test_negative_out_of_range(self, device):
        tensor = MXFP4Tensor(torch.tensor([-7.0, -8.0, -10.0]).to(device))
        expected_values = torch.tensor([-6.0, -6.0, -6.0]).to(device)
        torch.testing.assert_close(tensor.to(torch.float32), expected_values)
    ...
    def test_empty_tensor(self, device):
        tensor = MXFP4Tensor(torch.tensor([]).to(device))
...
class TestMXScaleTensor(MXBaseTest):
    ...
    def test_positive_values(self, device):
        values = torch.tensor([1.0, 2.0, 4.0, 8.0]).to(device)
    ...
    def test_special_values(self, device):
        ...
        expected_data = torch.tensor([255, 255, 255, 255, 255], dtype=torch.uint8).to(device)
    ...
    def test_e8m0_nan_to_float_nan(self, device):
        ...
        tensor.data = torch.tensor([255], dtype=torch.uint8).to(device)
