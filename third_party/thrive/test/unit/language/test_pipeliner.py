...
from triton._internal_testing import is_cuda, is_hopper_or_newer, is_hip_cdna, is_hip_cdna2, is_hip, is_blackwell
...
def test_pipeline_matmul(scale, device):
    ...
    if scale and device == "thrive":
        pytest.skip(reason="[Thrive] not supported on thrive")
    ...
    if scale:
        ...
        a = torch.randint(256, (M, K // DIV_FACTOR), dtype=torch.uint8).to(device)
        # Sample small-ish scales to avoid overflow
        scale_a = torch.randint(74, (M, K // 32), dtype=torch.uint8).to(device)
        ...
        if b_type == "bf16":
            b = torch.randn((K, N), dtype=torch.bfloat16).to(device)
        else:
            b = torch.randint(256, (K, N), dtype=torch.uint8)
            # e5m2 has too many non-finite values when sampled uniformly (1 / 32) and
            # Fp8E5M2_to_Bf16 doesn't preserve NaNs (fixme)
            finite = torch.arange(K * N, dtype=torch.uint8).to(device).reshape(K, N) % 0x7C
            b = torch.where(b & 0x7C == 0x7C, finite | (0x80 & b), b).to(device)
        output = torch.empty((M, N), dtype=torch.bfloat16).to(device)
    else:
    ...
    torch.testing.assert_close(ref_out, output.cpu(), atol=atol, rtol=rtol, equal_nan=scale)
    if is_cuda():
...
def test_pipeline_vecadd(device):
    ...
    BLOCK_SIZE = 256
    NUM_STAGES = 3
    a = torch.randn(SIZE, dtype=torch.float16).to(device)
    b = torch.randn(SIZE, dtype=torch.float16).to(device)
    output = torch.empty(SIZE, dtype=torch.float16).to(device)
    grid = (triton.cdiv(SIZE, NUM_BLOCKS * BLOCK_SIZE), 1)
    handler = vecadd_kernel[grid](a, b, output, SIZE, NUM_BLOCKS, BLOCK_SIZE, NUM_STAGES)
    ref_out = a.cpu() + b.cpu()
    torch.testing.assert_close(ref_out, output.cpu())
    if is_cuda():
        ttgir = handler.asm["ttgir"]
...
def test_pipeline_epilogue(ROW_COUNT, NUM_STAGES, device):
    ...
    width = ROW_COUNT
    depth = 78
    x = torch.zeros(width, depth)
    y0 = torch.rand_like(x).to(device)
    x = x.to(device)
    n_rows, n_cols = x.shape
    BLOCK_SIZE = triton.next_power_of_2(n_cols)
    kernel_up[(1, )](y0, x, x.stride(0), y0.stride(0), n_rows, n_cols, BLOCK_SIZE, NUM_STAGES)
    assert (y0.cpu() == torch.ones_like(x.cpu())).all()
...
def test_indirect_matmul(BLOCK_M, BLOCK_N, BLOCK_K, num_stages, device):
    ...
    Out = torch.empty((N, M), dtype=torch.float32).to(device)

    expect = torch.matmul(A.cpu().mT.to(torch.float32), B.cpu().to(torch.float32))
    ...
    torch.testing.assert_close(expect, Out.cpu())


@triton.jit
def matmul_kernel_persistent_scatter(
    a_ptr,
    b_ptr,
    c_ptr,  #
    M,
...
@pytest.mark.skipif(not is_blackwell(), reason="TMA Scatter only works on cloud Blackwell Chips")
def test_scatter_pipeline(device):
    ...
    a = torch.randn(M, K, dtype=torch.float16).to(device)
    b = torch.randn(N, K, dtype=torch.float16).to(device)
    ...
    ref = torch.matmul(a.cpu(), b.cpu().T)
    torch.testing.assert_close(c.cpu(), ref)
...
def test_conditional_store_pipeline(num_stages, device):
    ...
    N = 17
    arange = torch.arange(N, dtype=torch.int32).to(device)
    output = torch.zeros((N, ), dtype=torch.int32).to(device)
