...
def test_warp_specialize_tma_matmul(M, N, K, BLOCK_SIZE_M, BLOCK_SIZE_N, BLOCK_SIZE_K, num_stages, num_warps, use_fp8):
    ...
    A = torch.randn((M, K), dtype=torch.float16).to(device).to(dtype)
    B = torch.randn((N, K), dtype=torch.float16).to(device).to(dtype)
    C = torch.randn((M, N), dtype=torch.float16).to(device).to(dtype)
    ...
    ref_out = torch.empty((M, N), dtype=dtype).to(device)
...
def test_warp_specialize_attention_forward(M, N, BLOCK_M, HEAD_DIM, num_stages, disable_acc_multibuf, num_warps,
    ...
    torch.testing.assert_close(l_i.to(torch.float32).cpu(), l_i_ref.to(torch.float32).cpu(), atol=0, rtol=0)
