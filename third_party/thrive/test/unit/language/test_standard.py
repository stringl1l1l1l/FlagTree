...
from triton.backends.thrive.test.unit.language.test_core import _test_binary, int_dtypes, uint_dtypes, float_dtypes, numpy_random
...
def test_sort(M, N, k, descending, dtype_str, device):
    ...
    z = torch.empty(z_shape, dtype=x.dtype).to(device)
    if k is None or x.numel() < k:
        y = torch.sort(x.cpu(), descending=descending)[0]
    else:
        y = torch.topk(x.cpu(), k=k).values
    sort_kernel[(1, )](x, x.stride(0), z, z.stride(0), M, N, k, descending, num_warps=8)
    assert (y == z.cpu()).all(), (y, z.cpu())
...
def test_flip(M, N, K, dtype_str, dim, device):
    ...
    y = torch.flip(x, (dim, ))
    z = torch.empty_like(x).to(device)
    flip_kernel[(1, )](x.to(device), z, M, N, K, dim, num_warps=8)
    assert (y == z.cpu()).all(), (y, z.cpu())
...
def test_flip_inf(device):
    ...
    x = torch.arange(0, 16).unsqueeze(0).float()
    x[:, -1] = float("inf")

    expect = x.reshape(-1, 8, 2).flip(-1).reshape(-1, 16)
    actual = torch.empty_like(x).to(device)
    triton_flip_kernel[(x.shape[0], )](actual, x.to(device), x.shape[1])

    torch.testing.assert_close(expect, actual.cpu())
...
def test_ravel(device):
    ...
    out = torch.empty((256, ), dtype=torch.int32).to(device)
    triton_ravel[(1, )](out)

    assert (out.cpu() == torch.arange(0, 256)).all()
...
def test_swizzle2d(size_i, size_j, size_g, device):
    ...
    assert (output.cpu() == expected_order).all(), (output.cpu(), expected_order)
