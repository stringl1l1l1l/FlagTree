...
def test_print(func: str, data_type: str, device: str):
    ...
    x = torch.arange(0, N, dtype=torch.int32).to(getattr(torch, data_type)).to(device)
    y = torch.zeros((N, ), dtype=x.dtype).to(device)
    if func == "device_print":
        kernel_device_print[(1, )](x, y, BLOCK=N)
    elif func == "device_print_scalar":
        scalar = torch.tensor(42, dtype=x.dtype).to(device)
        kernel_device_print_scalar[(1, )](scalar)
    elif func == "device_print_negative":
        x = -x
        kernel_device_print[(1, )](x, y, BLOCK=N)
    elif func == "device_print_uint":
        x = torch.arange((1 << 31), (1 << 31) + N).to(device).to(getattr(torch, data_type))
        kernel_device_print[(1, )](x, y, BLOCK=N)
    elif func == "device_print_uint_cast":
        kernel_device_print_cast[(1, )](BLOCK=N)
    elif func == "print":
        kernel_print[(1, )](x, y, BLOCK=N)
    elif func == "device_print_large":
        kernel_device_print_large[(1, 2)](BLOCK_M=64, BLOCK_N=N)
    elif func == "print_multiple_args":
        kernel_print_multiple_args[(1, )](x, y, BLOCK=N)
    elif func == "device_print_multiple_args":
        kernel_device_print_multiple_args[(1, )](x, y, BLOCK=N)
    elif func == "static_print":
        kernel_static_print[(1, )](x, y, BLOCK=N, PLACEHOLDER=uuid.uuid4())
    elif func == "no_arg_print":
        kernel_no_arg_print[(1, )]()
    elif func == "print_no_arg":
        kernel_print_no_arg[(1, )]()
    elif func == "device_print_hex":
        kernel_device_print_hex[(1, )](x, y, BLOCK=N)
    elif func == "device_print_pointer":
        kernel_print_pointer[(1, )](x, y, BLOCK=N)
    elif func == "device_print_2d_tensor":
        BLOCK_SIZE_X = N
        BLOCK_SIZE_Y = N
        x_2d_tensor = x.reshape((BLOCK_SIZE_X, BLOCK_SIZE_Y))
        kernel_print_2d_tensor[(1, )](x_2d_tensor, y, BLOCK_SIZE_X=BLOCK_SIZE_X, BLOCK_SIZE_Y=BLOCK_SIZE_Y)
    else:
    ...
    excluded_funcs = {
        ...
        "device_print_large",
        "print_multiple_args",
        "device_print_multiple_args",
        "device_print_pointer",
        "device_print_scalar",
        "device_print_2d_tensor",
        "device_print_uint_cast",
    }
