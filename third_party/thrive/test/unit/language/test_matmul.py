...
from triton.backends.thrive.test.unit.language.test_mxfp import MXFP4Tensor, MXScaleTensor
import re
...
def f8_to_f16(x, dtype):
    ...
    ret = torch.empty(x.shape, dtype=torch.float16).to(x.device)
...
def test_simple_matmul(
    ...
    if dtype_src_str == "float8e5":
        ...
        b = torch.randint(20, 40, (K, N), dtype=torch.uint8).view(torch.float8_e5m2).to(device)
        A = f8_to_f16(a, dtype_src_str)
        B = f8_to_f16(b, dtype_src_str)
    else:
        dtype_src = getattr(torch, dtype_src_str)
        a = torch.randn(M, K, dtype=dtype_src).to(device)
        b = torch.randn(K, N, dtype=dtype_src).to(device)
        A = a
        B = b
    ...
    output = torch.empty((M, N), dtype=dtype_dst).to(device)
...
@pytest.mark.parametrize(
    ...
    [(128, 128, 16), (64, 128, 32), (32, 32, 32), (256, 128, 16), (64, 512, 16), (512, 64, 16), (64, 16, 16)],
)
@pytest.mark.parametrize("NUM_WARPS", [4, 8])
...
def test_simple_persistent_matmul(BLOCK_M, BLOCK_N, BLOCK_K, NUM_WARPS, DISALLOW_ACC_MULTI_BUFFER, device):
    ...
    output = torch.empty((M, N), dtype=torch.float16).to(device)
    ...
    ref_out = torch.matmul(a.cpu().to(torch.float32), b.cpu().to(torch.float32)).to(torch.float16)

    torch.testing.assert_close(ref_out, output.cpu(), atol=0.01, rtol=0.01)
...
def test_mxfp(BLOCK_M, BLOCK_N, BLOCK_K, NUM_STAGES, nonKDim, NUM_WARPS, device):
    ...
    a = torch.randint(20, 40, (M, K), dtype=torch.uint8).to(device).view(torch.float8_e5m2)
    a_f16 = f8_to_f16(a, dtype_src_str)
    b = torch.randint(20, 40, (K, N), dtype=torch.uint8).to(device).view(torch.float8_e5m2)
    b_f16 = f8_to_f16(b, dtype_src_str)
    a_scale = torch.randint(64, 130, (M, K // 32), dtype=torch.uint8).to(device)
    b_scale = torch.randint(64, 130, (N, K // 32), dtype=torch.uint8).to(device)
...
@triton.jit
def _gemm_kernel_preshuffled_scales_cdna4(
    a_ptr,
    b_ptr,
    c_ptr,
    a_scales_ptr,
...
@pytest.mark.parametrize(
    "DTYPE_A, DTYPE_B, FAST_MATH",
    [("mxfp4", "mxfp4", False), ("fp16", "mxfp8e5", False), ("mxfp8e4", "bf16", False), ("bf16", "mxfp4", True)],
)
...
def test_preshuffle_scale_mxfp_cdna4(M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, DTYPE_A, DTYPE_B, FAST_MATH, mfma_nonkdim,
    ...
    dtype_to_torch_type = {
        "fp16": torch.half,
        "bf16": torch.bfloat16,
        "mxfp8e5": torch.float8_e5m2,
        "mxfp8e4": torch.float8_e4m3fn,
    }
    ...
    def generate_gemm_input(dim0, dim1, dtype):
        ...
        if dtype == "mxfp4":
            ...
            v = torch.randn((dim0, dim1), dtype=dtype_to_torch_type[dtype]).to(device)
        else:
            raise ValueError(f"Unsupported data type: {dtype}")

        if dtype.startswith("mx"):
            scales = torch.randint(124, 128, (dim0, dim1 // SCALE_GROUP_SIZE), dtype=torch.uint8).to(device)
...
def test_lhs_in_tmem(BLOCK_M, BLOCK_N, BLOCK_K, a_trans, dtype_src_str, device, monkeypatch):
    ...
    torch.manual_seed(42)
    if dtype_src_str == "float8e5":
        a = torch.randint(20, 40, (M, K), dtype=torch.int8).view(torch.float8_e5m2)
        b = torch.randint(20, 40, (K, N), dtype=torch.int8).view(torch.float8_e5m2)
        if a_trans:
            a = a.T.contiguous().T
        a = a.to(device)
        b = b.to(device)
        ...
        dtype_src = getattr(torch, dtype_src_str)
        a = torch.randn(M, K, dtype=dtype_src)
        b = torch.randn(K, N, dtype=dtype_src)
        if a_trans:
            a = a.T.contiguous().T
        a = a.to(device)
        b = b.to(device)
        A = a
        B = b
    output = torch.empty((M, N), dtype=torch.float32).to(device)
    grid = (triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1)
    k = matmul_kernel[grid](
        a,
        b,
    ...
    atol = 0.03
    rtol = 0.03
    torch.testing.assert_close(ref_out, output.cpu(), atol=atol, rtol=rtol)
...
def test_lhs_in_tmem_mxfp(device, monkeypatch):
    ...
    M, N, K = 128, 64, 32
    torch.manual_seed(42)
    a = torch.randint(20, 40, (M, K), dtype=torch.uint8).to(device)
    b = torch.randint(20, 40, (K, N), dtype=torch.uint8).to(device)
    A = f8_to_f16(a, "float8e5")
    B = f8_to_f16(b, "float8e5")
    a_scale = torch.randint(124, 130, (M, K // 32), dtype=torch.uint8).to(device)
    b_scale = torch.randint(124, 130, (N, K // 32), dtype=torch.uint8).to(device)
    output = torch.empty((M, N), dtype=torch.float16).to(device)
    grid = (1, 1)
    lhs_in_tmem_kernel_mxfp[grid](
        a,
        b,
        output,
        a_scale,
...
def test_block_scale_fp4(M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, VEC_SIZE, with_a_scale, with_b_scale, pack_along_k,
    ...
    a_scale = torch.rand(a_size).to(device)
    b_scale = torch.rand(b_size).to(device)
    if scale_type == "float8_e8m0fnu":
...
def test_mxfp8_mxfp4_matmul(
    ...
    def create_operand(dtype: str, size0: int, size1: int, k_dim: int, transpose: bool = True,
        ...
        if dtype == "float8e5":
            if transpose:
                ...
                v = torch.randint(20, 40, (size1, size0), dtype=torch.uint8).view(torch.float8_e5m2).T.to(device)
                v_ref = f8_to_f16(v.view(torch.float8_e5m2).T, dtype).cpu().to(torch.float32).T
        elif dtype == "float8e4nv":
            if transpose:
                v = torch.randint(20, 40, (size0, size1), dtype=torch.uint8).view(torch.float8_e4m3fn).to(device)
                v_ref = f8_to_f16(v.view(torch.float8_e4m3fn), dtype).cpu().to(torch.float32)
            else:
                v = torch.randint(20, 40, (size1, size0), dtype=torch.uint8).view(torch.float8_e4m3fn).T.to(device)
                v_ref = f8_to_f16(v.view(torch.float8_e4m3fn).T, dtype).cpu().to(torch.float32).T
        else:
            # float4
            if pack_along_k:
                pack_dim = k_dim
                ...
                pack_dim = (k_dim + 1) % 2
            if transpose:
                v_mxfp4 = MXFP4Tensor(size=(size0, size1)).random()
                v = v_mxfp4.to_packed_tensor(dim=pack_dim).to(device)
                v_ref = v_mxfp4.to(torch.float32)
            else:
                v_mxfp4 = MXFP4Tensor(size=(size1, size0)).random()
                v = v_mxfp4.to_packed_tensor(dim=(pack_dim + 1) % 2).T.to(device)
    ...
    a_scale_mxfp4 = MXScaleTensor(size=(M, (K + 32 - 1) // 32), device=device).random(high=32.0).to(device)
    b_scale_mxfp4 = MXScaleTensor(size=(N, (K + 32 - 1) // 32), device=device).random(high=32.0).to(device)
    a_scale = a_scale_mxfp4.data
    b_scale = b_scale_mxfp4.data

    a_scale_ref = a_scale_mxfp4.cpu().to(torch.float32).repeat_interleave(32, dim=1)[:M, :K]
    if CONST_SCALE:
        a_scale_ref = torch.full_like(a_scale_ref, 2.0)
        a_scale = 128  # 2.0 in e8m0
    b_scale_ref = b_scale_mxfp4.cpu().to(torch.float32).repeat_interleave(32, dim=1).T.contiguous()[:K, :N]
    ...
    output = a.new_empty((M, N), dtype=torch.float32).to(device)
    grid = (triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1)
    kernel_kwargs = {}
    if is_hip():
        kernel_kwargs["matrix_instr_nonkdim"] = nonKDim
    out = mxfp8_mxfp4_matmul[grid](
        ...
        output.stride(1),
        not CONST_SCALE,
        dtype_converter[A_DATA_TYPE],
        dtype_converter[B_DATA_TYPE],
        BLOCK_M,
        BLOCK_N,
        BLOCK_K,
        PACK_B_ALONG_K=PACK_B_ALONG_K,
        NUM_STAGES=NUM_STAGES,
        **kernel_kwargs,
    )
    if is_cuda():
