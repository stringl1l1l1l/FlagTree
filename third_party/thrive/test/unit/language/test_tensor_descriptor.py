...
def test_tensor_descriptor_load(dtype_str, num_ctas, M_BLOCK, N_BLOCK, device):
    ...
    def alloc_fn(size: int, align: int, stream: Optional[int]):
        ...
        assert align == 128
        assert stream == 0
        return torch.empty(size, dtype=torch.int8).to(device)
...
def test_tensor_descriptor_padding(device):
    ...
    def alloc_fn(size: int, alignment: float, stream: float):
        return torch.ones(size, dtype=torch.float32).to(device)
    ...
    N_BLOCK = 32
    padding = "nan"
    input = torch.arange(IM * IN, dtype=torch.float32).to(device)
    input = input.reshape(IM, IN)
    out_device_tma = torch.zeros((OM, ON), dtype=torch.float32).to(device)
    out_host_tma = torch.zeros((OM, ON), dtype=torch.float32).to(device)
    ...
    expected = torch.zeros((OM, ON), dtype=torch.float32).to(device)
    ...
    torch.testing.assert_close(expected.cpu(), out_host_tma.cpu(), equal_nan=True)
...
def test_make_tensor_descriptor_matmul(num_stages, num_ctas, BLOCK_M, BLOCK_N, BLOCK_K, device):
    ...
    C = torch.empty((M, N), dtype=torch.float16).to(device)
    ...
    if torch.cuda.get_device_capability(0)[0] >= 9:
        assert ("tensormap.cp_fenceproxy.global.shared::cta.tensormap::generic.release.gpu.sync.aligned"
                in kernel.asm["ptx"])
    if BLOCK_M >= 64 * num_ctas and BLOCK_N >= 64 and is_hopper():
        ...
        assert ("stmatrix.sync.aligned.m8n8.x4.shared.b16" in kernel.asm["ptx"]
                or "stmatrix.sync.aligned.x4.m8n8.shared.b16" in kernel.asm["ptx"])
...
def test_make_tensor_descriptor_loop_carried(device):
    M, N = 64, 512
    torch.manual_seed(42)
    A = torch.randn((M, N), dtype=torch.float32).to(device)
    MBLOCK, NBLOCK = 8, 128
...
@triton.jit
def batched_gemm_2d_tma_kernel(
    a_ptr,
    b_ptr,
    c_ptr,  #
    B,
...
def test_tensor_descriptor_batched_gemm_2d_tma(device):
    ...
    a = torch.randn((B, M, K), dtype=torch.float16).to(device)
    b = torch.randn((B, N, K), dtype=torch.float16).to(device)
    c = torch.empty((B, M, N), dtype=torch.float16).to(device)
...
@triton.jit
def batched_gemm_3d_tma_kernel(
    a_ptr,
    b_ptr,
    c_ptr,  #
    B,
...
@triton.jit()
def matmul_kernel_rank_reducing(
    a_ptr,
    b_ptr,
    c_ptr,  #
    M,
...
def test_tma_gather(X, Y, BLOCK_X, BLOCK_Y, dtype, y, device):
    ...
    torch.manual_seed(42)
    if dtype != torch.int8:
        input = torch.rand((X, Y), dtype=dtype).to(device)
    else:
        input = torch.arange(X * Y, dtype=dtype).to(device).reshape(X, Y)
    output = torch.empty((BLOCK_X, BLOCK_Y), dtype=dtype).to(device)

    idx = torch.randint(BLOCK_X, (BLOCK_X, ), dtype=torch.int32).to(device)
    a = torch.arange(BLOCK_M * K).to(device).reshape(BLOCK_M, K).float()
    b = torch.arange(K * BLOCK_N).to(device).reshape(K, BLOCK_N).float()
    output = torch.zeros((BLOCK_M, BLOCK_N), dtype=torch.float32).to(device)
...
def test_tma_gather_dot_pipeline(BLOCK_M, BLOCK_N, BLOCK_K, K, device):
    ...
    if is_native_gather:
        kernel = tma_gather_dot_pipeline.warmup(
            a,
            b,
            output,
            a.stride(0),
...
def test_tma_scatter(X, Y, BLOCK_X, BLOCK_Y, dtype, y, device):
    ...
    torch.manual_seed(42)
    input = torch.arange(BLOCK_X * BLOCK_Y, dtype=dtype).to(device).reshape(BLOCK_X, BLOCK_Y)
    output = torch.zeros((X, Y), dtype=dtype).to(device)
    idx = torch.randperm(BLOCK_X, dtype=torch.int32).to(device)
...
def test_tensor_descriptor_store_downcast(dtype_str, device):
    ...
    grid_m = M // M_BLOCK
    grid_n = N // N_BLOCK
    out = torch.empty((M, N), dtype=torch_dtype).to(device)
    ...
    ref = torch.arange(M * N, dtype=torch.float32).to(device).reshape(M, N).to(torch_dtype)
