...
def to_triton_thrive(x: np.ndarray, device, dst_type=None) -> Union[TensorWrapper, torch.Tensor]:
    '''
    ...
    if t in uint_dtypes:
        ...
        return reinterpret(torch.tensor(x_signed).to(device), getattr(tl, t))
    else:
        if dst_type and 'float8' in dst_type:
            return reinterpret(torch.tensor(x).to(device), getattr(tl, dst_type))
...
def _test_unary(dtype_x, expr, numpy_expr=None, device="cuda", num_ctas=1):
    ...
    # triton result
    x_tri = to_triton_thrive(x, device=device, dst_type=dtype_x)
    z_tri = to_triton_thrive(np.empty_like(x), device=device, dst_type=dtype_x)
...
def _test_binary(
    ...
    def do_test(x, y, kernel_fn):
        ...
        # triton result
        x_tri = x if x_is_scalar else to_triton_thrive(x, device=device, dst_type=dtype_x)
        y_tri = y if y_is_scalar else to_triton_thrive(y, device=device, dst_type=dtype_y)
        z_tri = to_triton_thrive(np.empty(SIZE, dtype=z_ref.dtype), device=device)
        kernel_fn[(1, )](z_tri, x_tri, y_tri, SIZE=SIZE, num_warps=4, num_ctas=num_ctas)
        err_msg = f"{expr}, {kernel_fn.__name__}"
        np.testing.assert_allclose(z_ref, to_numpy(z_tri.cpu()), err_msg=err_msg, atol=7e-3, rtol=0.01)
...
def test_addptr(dtype, order, device):
    ...
    x_tri = to_triton_thrive(x, dst_type=dtype, device=device)
    y_tri = to_triton_thrive(y, dst_type=dtype, device=device)
    y = x
    kernel[
        1,
...
def test_unsigned_name_mangling(device):
    ...
    # triton result
    x_tri = to_triton_thrive(x, device=device, dst_type=dtype_x)
    y_tri = to_triton_thrive(y, device=device, dst_type=dtype_y)
    actual = tuple(to_triton_thrive(np.empty_like(e), device=device) for e in expect)
    kernel[(1, )](actual[0], actual[1], x_tri, y_tri, SIZE=SIZE, num_warps=4)

    # Bitwise op, so expect exact equality
    assert (expect[0] == to_numpy(actual[0].cpu())).all()
    assert (expect[1] == to_numpy(actual[1].cpu())).all()
...
def test_broadcast(dtype, device):
    ...
    x_tri = to_triton_thrive(x, device=device, dst_type=dtype)
    y_tri = to_triton_thrive(y, device=device, dst_type=dtype)
    y_broadcasted_tri = to_triton_thrive(np.empty((M, N), dtype=y_broadcasted_np.dtype), device=device, dst_type=dtype)
...
def test_expand_dims(device):
    ...
    N = 32
    dummy_tensor = torch.empty(()).to(device)
...
def test_where(dtype, num_ctas, device):
    ...
    cond_tri = to_triton_thrive(cond, device=device)
    x_tri = to_triton_thrive(x, device=device, dst_type=dtype)
    y_tri = to_triton_thrive(y, device=device, dst_type=dtype)
    z_tri = to_triton_thrive(np.empty(SIZE, dtype=z.dtype), device=device, dst_type=dtype)
    ...
    if select_ptrs:
        ...
        assert (z == to_numpy(z_tri.cpu())).all()
...
def test_where_broadcast(num_ctas, device):
    ...
    z = np.where(mask, x, 0)
    cond_tri = to_triton_thrive(mask, device=device)
    x_tri = to_triton_thrive(x, device=device, dst_type=dtype)
    z_tri = to_triton_thrive(np.empty((SIZE, SIZE), dtype=z.dtype), device=device, dst_type=dtype)
...
def test_math_erf_op(dtype, device):
    ...
    kernel[(1, )](z_tri, x.to(device), SIZE=SIZE, num_warps=4)
    torch.testing.assert_close(z_tri.cpu(), z_ref.cpu())
...
def test_math_fma_op(dtype, device):
    ...
    z_ref = x * y + w
    z_tri = torch.zeros_like(x).to(device)
    kernel[(1, )](z_tri, x.to(device), y.to(device), w.to(device), SIZE=SIZE, num_warps=4)
...
def test_precise_math(expr_prec, expr_ref, num_ctas, device):
    ...
    y = torch.randn(shape, dtype=torch.float32).to(device)
    ...
    assert torch.all(out.cpu() == out_ref.cpu())  # bitwise exact
...
def test_abs_fp8(in_dtype, device):
    if is_hip():
        ...
        if in_dtype == tl.float8e4b15:
            pytest.skip("[Thrive] not supported on thrive")
    ...
    if device == "thrive":
        f8_tensor = f8_tensor.to(device)
    f8 = triton.reinterpret(f8_tensor, in_dtype)
    n_elements = f8_tensor.numel()
    out_f8 = torch.empty_like(f8_tensor).to(device)
    abs_kernel[(1, )](f8.to(device), triton.reinterpret(out_f8, in_dtype), n_elements)

    # avoid directly conversion from f8 to f32 which not supported in torch_thrive
    if device == "thrive":
        f8_tensor = f8_tensor.to("cpu")
...
def test_transpose(dtype_x, device):
    ...
    z_ref = x.T
    x_tri = to_triton_thrive(x, device=device, dst_type=dtype_x)
    z_tri = to_triton_thrive(np.empty_like(z_ref), device=device, dst_type=dtype_x)
...
def test_index1d(expr, dtype_str, num_ctas, device):
    ...
    z_ref = eval(expr) + y
    # triton result
    z_tri = to_triton_thrive(np.empty_like(z_ref), device=device)
    x_tri = to_triton_thrive(x, device=device)
...
def test_noinline(mode, device):
    ...
    x = torch.tensor([1.0], dtype=torch.float32).to(device)
    y = torch.tensor([2.0], dtype=torch.float32).to(device)
    if mode == "shared":
        z = torch.ones((16, 16), dtype=torch.float32).to(device)
    else:
        z = torch.tensor([0.0], dtype=torch.float32).to(device)
    ...
    if mode == "simple":
        ...
        ref = torch.full((16, 16), 16, dtype=torch.float32).to(device)
...
def test_atomic_rmw(op, dtype_x_str, mode, sem, device):
    ...
    if device == "thrive":
        if (dtype_x_str in ["float16", "bfloat16", "float32", "float64"]):
            pytest.skip("[Thrive] hardware does not support float atomic RMW")
    ...
    if mode == "max_pos":
        idx = rs.randint(n_programs, size=(1, )).item()
        x[idx] = np.max(np.abs(x)) + 1
    x_tri = to_triton_thrive(x, device=device, dst_type=dst_type)

    z_tri = to_triton_thrive(np.array([neutral], dtype=getattr(np, dtype_x_str)), device=device, dst_type=dst_type)
    ...
    exact = op not in ["add"]
    if exact:
        assert z_ref.item() == to_numpy(z_tri.cpu()).item()
    else:
        np.testing.assert_allclose(z_ref, to_numpy(z_tri.cpu()), rtol=0.01)
...
def test_tensor_atomic_rmw(shape, axis, num_ctas, dtype_x_str, check_return_val, device):
    ...
    old_ref = np.copy(z)
    # triton result
    x_tri = to_triton_thrive(x, device=device, dst_type=dtype_x_str)
    z_tri = to_triton_thrive(z, device=device, dst_type=dtype_x_str)
    old_tri = to_triton_thrive(old, device=device, dst_type=dtype_x_str)
    ...
    kernel[(1, )](
        ...
        shape0,
        shape1,
        torch_to_triton_dtype(x_tri.dtype),
        check_return_val,
        num_ctas=num_ctas,
    )
    ...
    if dtype_x_str == "bfloat16":
        ...
        np.testing.assert_allclose(z_ref, to_numpy(z_tri.cpu()), rtol=0.5)
        # check return vals, but use assert_allclose for bf16
        if check_return_val:
            np.testing.assert_allclose(old_ref, to_numpy(old_tri.cpu()), rtol=0.5)
        return

    np.testing.assert_allclose(z_ref, to_numpy(z_tri.cpu()), rtol=1e-4)
    if check_return_val:
        np.testing.assert_equal(old_ref, to_numpy(old_tri.cpu()))


@pytest.mark.skip(reason="[Thrive] hardware does not support float atomic")
@pytest.mark.interpreter
@pytest.mark.parametrize(
...
def test_tensor_atomic_add_non_exclusive_offset(size, num_ctas, dtype_x_str, device):
    ...
    x = torch.zeros(shape, dtype=dtype).to(device)
    val = torch.randn((size**2), dtype=dtype).to(device)
...
def test_tensor_atomic_add_shift_1(size, num_ctas, dtype_x_str, device):
    ...
    s = (2, size)
    dtype = getattr(torch, dtype_x_str)
    x = torch.zeros(s, dtype=dtype).to(device)
    ref = torch.flatten(x)
    val = torch.randn(s, dtype=dtype).to(device)
    kernel[(1, )](x, val, size, num_warps=1, num_ctas=num_ctas)
...
def test_tensor_atomic_add_access_patterns(shape, idx_order, mask_step, num_ctas, dtype_x_str, device):
    ...
    shape0, shape1 = shape
    idx_row = torch.arange(0, shape1).to(device)
    ...
    if idx_order == "random_no_duplication":
        idx = torch.stack([torch.randperm(shape1).to(device) for _ in idx_row])
    if idx_order == "random":
        idx = torch.randint(0, shape1, size=(shape0, shape1)).to(device)

    dtype = getattr(torch, dtype_x_str)
    val = torch.randn((shape0, shape1), dtype=dtype).to(device)
    dst = torch.randn((shape0, shape1), dtype=dtype).to(device)
...
def test_tensor_atomic_rmw_block(num_ctas, device):
    ...
    x = torch.ones((8, 8), dtype=torch.float32).to(device)
    kernel[(2, )](x, shape[0], shape[1], num_ctas=num_ctas)
    x_cpu = x.cpu()
    assert x_cpu.min().item() == 0.0
...
def test_atomic_cas(sem, num_ctas, dtype_str, device):
    ...
    if device == "thrive":
        if (dtype_str in ["float16", "bfloat16", "float32", "float64"]):
            pytest.skip("[Thrive] hardware does not support float atomic CAS")
    ...
    Lock = torch.zeros((1, ), dtype=torch_dtype).to(device)
    ...
    ref = torch.full((128, ), 100.0)
    h = serialized_add[(100, )](data, Lock, triton_dtype=triton_dtype, SEM=sem, num_ctas=num_ctas)
    sem_str = "acq_rel" if sem is None else sem
    np.testing.assert_allclose(to_numpy(data.cpu()), to_numpy(ref))
    if not is_cuda():
        return
...
def test_tensor_atomic_cas(sem, size, dtype_str, num_ctas, device):
    ...
    torch_dtype = getattr(torch, dtype_str)
    X_host = torch.zeros((size, ), dtype=torch_dtype)
    X_host[1::2] = 1
    X = X_host.to(device)
    Y = X_host.clone()
    Y[0::2] = 2
...
def test_atomic_min_max_neg_zero(device):
    if device == "thrive":
        pytest.skip("[Thrive] hardware does not support float atomic min/max")
    ...
    N_PROG = 1
    dtype = torch.float32
    out_min = torch.full([N_PROG], torch.finfo(torch.float32).max, dtype=dtype).to(device)
    out_max = torch.full([N_PROG], torch.finfo(torch.float32).min, dtype=dtype).to(device)
    inp = torch.full([N_PROG], -0.0, dtype=dtype).to(device)
    kernel[(N_PROG, )](inp, out_max, out_min)
    torch.testing.assert_close(out_min.cpu(), inp.cpu(), atol=0, rtol=0)
    torch.testing.assert_close(out_max.cpu(), inp.cpu(), atol=0, rtol=0)
...
def test_atomic_unsupported_type(dtype_str, device):
    if device == "thrive":
        if (dtype_str == "float8_e4m3fn"):
            pytest.skip("[Thrive] hardware does not support float atomic")
    ...
    I = torch.zeros((1, ), dtype=getattr(torch, dtype_str)).to(device)
    O = torch.zeros((1, ), dtype=getattr(torch, dtype_str)).to(device)
...
def test_tensor_atomic_use_result(dtype_str, size, op, device):
    ...
    index = torch.arange(0, size).to(dtype=getattr(torch, dtype_str)).to(device)
    out = torch.zeros((size, size), dtype=getattr(torch, dtype_str)).to(device)
    kernel[(1, )](index, out, size, op)
    assert (out.cpu() == 5).all()
...
def test_cast(dtype_x, dtype_z, bitcast, size, num_ctas, device):
    ...
    if dtype_x.startswith("bfloat"):
        x_tri = torch.randn(size, dtype=getattr(torch, dtype_x)).to(device)
    elif dtype_x.startswith("float8"):
        x_tri = torch.randn(size, dtype=torch.half).to(dtype=getattr(torch, dtype_x)).to(device)
    else:
    ...
    if "float" in dtype_z and "float" in dtype_x:
        ...
        x_tri = x_tri.cpu().to(getattr(torch, dtype_z)).to(getattr(torch, dtype_x)).to(device)
    # triton kernel
    ...
    if dtype_z.startswith("bfloat"):
        z_tri = torch.empty((size, ), dtype=getattr(torch, dtype_z)).to(device)
    elif dtype_z.startswith("float8"):
        z_tri = torch.empty((size, ), dtype=torch.half).to(dtype=getattr(torch, dtype_z)).to(device)
    else:
        z_tri = to_triton_thrive(np.empty((size, ), dtype=getattr(np, dtype_z_np)), device=device)
...
def test_cat(dtype_str, num_warps, device):
    ...
    x = torch.arange(0, 128).to(getattr(torch, dtype_str)).to(device)
    y = torch.arange(-128, 0).to(getattr(torch, dtype_str)).to(device)
...
def test_store_constant(num_ctas, dtype_str, constant_field, device):
    ...
    block_size = 128
    ref = torch.ones([block_size], dtype=getattr(torch, dtype_str)).to(device)
    ...
    if constant_field == "value":
        assert torch.all(output.cpu() == ref.cpu())
    else:
...
def test_load_store_same_ptr(device):
    ...
    for _ in range(100):
        x = torch.ones((32, ), dtype=torch.float32).to(device)
        if is_hip():
            kernel[(65536, )](x, num_warps=16)  # threads per Warp for ROCM is 64
        else:
            kernel[(32, )](x, num_warps=32)
...
def test_umulhi(dtype_str, device):
    ...
    y_tri = to_triton_thrive(y, device=device)
...
def test_join(device):
    ...
    kernel[(1, )](x.to(device), y.to(device), z, N=128)

    np.testing.assert_equal(to_numpy(z_ref), to_numpy(z.cpu()))
...
def test_join_scalars(device):
    ...
    np.testing.assert_equal([42, 100], to_numpy(z.cpu()))
...
def test_interleave_scalars(device):
    ...
    np.testing.assert_equal([10, 20], to_numpy(z.cpu()))
...
def test_split(device):
    ...
    kernel[(1, )](x.to(device), z1, z2, N=256)

    np.testing.assert_equal(to_numpy(z1_ref), to_numpy(z1.cpu()))
    np.testing.assert_equal(to_numpy(z2_ref), to_numpy(z2.cpu()))
...
def test_max_min_with_nan(device):
    ...
    BLOCK_SIZE = 64
    x = torch.rand((1, BLOCK_SIZE), dtype=torch.float32).to(device)
...
def test_reduce1d(op, dtype_str, shape, num_ctas, device):
    ...
    if "tie-break-left" not in op and dtype_str == "bfloat16":
        ...
        z_ref = numpy_op(x).astype(getattr(np, z_dtype_str))
    # triton result
    z_tri = to_triton_thrive(numpy_random((1, ), dtype_str=z_dtype_str), device=device, dst_type=z_tri_dtype_str)
...
def test_reduce(op, dtype_str, shape, axis, keep_dims, num_ctas, device):
    ...
    # triton result
    z_shape = z_ref.shape
    z_tri = to_triton_thrive(numpy_random(z_shape, dtype_str=z_dtype_str), device=device, dst_type=z_tri_dtype_str)
...
def test_sum_dtype(device):
    ...
    if device == "thrive":
        # [Thrive] known issues: vfredosum on bf16 gets wrong
        # result when element num > 256. Root cause from ESL.
        # Use shape (16, 16) instead of (32, 32) can succeed.
        return
    out = torch.empty(1, dtype=torch.bfloat16).to(device)
    kernel_default_float[(1, )](out)
    torch.testing.assert_close(out.cpu()[0], torch.tensor(32 * 32, dtype=torch.bfloat16))
...
def test_scan2d(op, dtype_str, shape, axis, reverse, num_warps, device):
    ...
    if reverse:
        x_in = np.flip(x, axis)
    z = np.empty_like(x)
    x_tri = to_triton_thrive(x, device=device, dst_type=dtype_str)
    y_tri = to_triton_thrive(y, device=device, dst_type=dtype_str)
    if op == "cumsum" or op == "cumprod":
    ...
    z_tri = to_triton_thrive(z, device=device)
...
def test_histogram(M, N, device):
    ...
    z = torch.empty(N, dtype=torch.int32).to(device)
    ...
    histogram_kernel[(1, )](x.to(device), z, M=M, N=N)
...
def test_histogram_silent_data_corruption(device):
    ...
    x = torch.ones(1, dtype=torch.int32).to(device)
    z = torch.ones(2, dtype=torch.int32).to(device)

    histogram_kernel[(1, )](x, z)
    assert z.cpu()[1] == 1, f"Second element shouldn't be affected, expected_buffer=[1, 1], actual_buffer={z}"
...
def test_histogram_mask(M, N, device):
    ...
    torch.manual_seed(17)
    x1 = torch.randint(0, N, (M, ), dtype=torch.int32)
    x = torch.cat((x1, x1), 0).to(device)
    z = torch.empty(N, dtype=torch.int32).to(device)
    # torch.histc does not work when the input type is not float and the device is CPU
    # https://github.com/pytorch/pytorch/issues/74236
    # This is a workload by converting the input to float
...
def test_scan_1d(M, N, device):
    ...
    x = torch.randint(-100, 100, (M, ), dtype=torch.int32)
    output = torch.empty(M * N, dtype=torch.int32).to(device)

    scan_kernel[(1, )](output, x.to(device), M, N)
...
@pytest.mark.skip(reason="[Thrive] not supported on thrive")
@pytest.mark.interpreter
...
def test_no_rematerialization_op():
    ...
    data_dim = 64
    torch.manual_seed(0)
    input_data = torch.randn((data_len, data_dim), dtype=torch.float32).to(device)
    sum_output = torch.full((data_len, ), -1, dtype=torch.float32).to(device)
    out_1 = torch.full((data_len, 2), -1, dtype=torch.float32).to(device)
...
def test_generic_reduction(device):
    ...
    SIZE = 512
    x = torch.rand(SIZE)
    out_mean = torch.empty(()).to(device)
    out_var = torch.empty(()).to(device)
    sum0 = torch.empty(()).to(device)
    sum1 = torch.empty(()).to(device)

    var_mean_kernel[(1, )](x.to(device), out_mean, out_var, sum0, sum1, BLOCK=SIZE)

    expect_var, expect_mean = torch.var_mean(x, dim=0, correction=0)
    sum_ref = torch.sum(x)
    torch.testing.assert_close(out_mean.cpu(), expect_mean)
    torch.testing.assert_close(out_var.cpu(), expect_var)
    torch.testing.assert_close(sum0.cpu(), sum_ref)
    torch.testing.assert_close(sum1.cpu(), sum_ref)
...
def test_permute(dtype_str, shape, perm, num_ctas, device):
    check_type_supported(dtype_str, device)  # bfloat16 on cc < 80 will not be tested
    if dtype_str == "float8e4b15" and (device == "thrive"):
        pytest.skip("[Thrive] not supported on thrive")
    ...
    # triton result
    z_tri = to_triton_thrive(np.empty_like(x), device=device, dst_type=dtype_str)
    z_tri_contiguous = to_triton_thrive(np.empty_like(x), device=device, dst_type=dtype_str)
    x_tri = to_triton_thrive(x, device=device, dst_type=dtype_str)
    pgm = kernel[(1, 1)](
        x_tri,
        x_tri.stride(0),
...
def test_trans_2d(dtype_str, shape, perm, device):
    ...
    input = torch.arange(math.prod(shape), dtype=getattr(torch, dtype_str)).reshape(shape).to(device)
...
def test_trans_4d(dtype_str, shape, perm, device, with_allocator):
    ...
    def kernel(
        ...
        off1 = tl.arange(0, in_shape1)
        off2 = tl.arange(0, in_shape2)
        off3 = tl.arange(0, in_shape3)
        off4 = tl.arange(0, in_shape4)
        stride_in4 = 1
        stride_in3 = in_shape4
        stride_in2 = in_shape4 * in_shape3
        stride_in1 = in_shape4 * in_shape3 * in_shape2
        idx1 = off1[:, None, None, None]
        idx2 = off2[None, :, None, None]
        idx3 = off3[None, None, :, None]
        idx4 = off4[None, None, None, :]
        offsets_in = (idx1 * stride_in1 + idx2 * stride_in2 + idx3 * stride_in3 + idx4 * stride_in4)
        val = tl.load(In + offsets_in)
        val_permuted = tl.permute(val, (trans1, trans2, trans3, trans4))
        out_total_size: tl.constexpr = ou_shape1 * ou_shape2 * ou_shape3 * ou_shape4
        offsets_out = tl.arange(0, out_total_size)
        val_flat = tl.reshape(val_permuted, [out_total_size])
        tl.store(Out + offsets_out, val_flat)
...
def convert_fp8_to_fp32(x, device, dtype_str):
    if dtype_str == "float8e4nv":
        return torch.tensor(x).view(torch.float8_e4m3fn).to(torch.float32).to(device)
    elif dtype_str == "float8e5":
        return torch.tensor(x).view(torch.float8_e5m2).to(torch.float32).to(device)
    elif dtype_str == "float8e4b8":
        return torch.tensor(x).view(torch.float8_e4m3fnuz).to(torch.float32).to(device)
    elif dtype_str == "float8e5b16":
        return torch.tensor(x).view(torch.float8_e5m2fnuz).to(torch.float32).to(device)
...
def get_test_small_dots_cases():
    if not is_cuda():
        return []
    return [
        (2, 4, 32, 1, False, False, "None", "ieee", "float16", "float32", 1, None),
...
def test_dot(
    ...
    if is_interpreter():
        ...
        if device == "thrive":
            if in_dtype == "float64" or out_dtype == "float64":
                pytest.skip("float64 not supported on Thrive")
            if input_precision not in ("tf32", "ieee"):
                pytest.skip(f"{input_precision} not supported on Thrive")
    ...
    x_tri = to_triton_thrive(x, device=device, dst_type=in_dtype)
    y_tri = to_triton_thrive(y, device=device, dst_type=in_dtype)
    w_tri = to_triton_thrive(w, device=device, dst_type=in_dtype)
    # triton result
    if out_dtype == "int8":
        z = 1 + numpy_random((M, N), dtype_str="int32", rs=rs)
    else:
        z = 1 + numpy_random((M, N), dtype_str=out_dtype, rs=rs) * 0.1
    ...
    if in_dtype == "int8":
        ...
        x = convert_fp8_to_fp32(x, "cpu", in_dtype)
        y = convert_fp8_to_fp32(y, "cpu", in_dtype)
...
def test_scaled_dot(M, N, K, col_a, col_b, rhs_scale, mxfp_type, normal_type, num_warps, mma, kpack, device):
    ...
    def make_arg(shape, ty, col_major=False):
        ...
        if ty == "bf16" or ty == "fp16":
            ret = torch.randn(shape, dtype=comp_dtype).to(device)
            ...
            if is_hip_cdna4():
                ...
                ret = torch.randint(20, 40, shape, dtype=torch.uint8).to(device)
            else:
                ret = torch.randint(256, shape, dtype=torch.uint8).to(device)
        if col_major:
            ret = ret.mT
        return ret
    ...
    scale_x = torch.randint(min_scale, max_scale + 1, (M, K // 32), dtype=torch.uint8).to(device)
    scale_y = torch.randint(min_scale, max_scale + 1, (N, K // 32), dtype=torch.uint8).to(device)
    if rhs_scale:
        scale_x = None
    else:
        scale_y = None
    ...
    def make_finite(x, dtype):
        ...
        finite = torch.arange(x.numel(), dtype=torch.uint8).reshape_as(x).to(device) % mask
...
def test_dot3d(B, num_warps, M, N, K, BLOCK_M, BLOCK_N, in_dtype_str, out_dtype_str, device):
    ...
    x_tri = to_triton_thrive(x, device=device)
    y_tri = to_triton_thrive(y, device=device)
    out_tri = to_triton_thrive(out, device=device)
...
def test_dot_mulbroadcasted(in_dtype, device):
    ...
    z_ref = np.matmul(x, y)
    np.testing.assert_allclose(z_ref, z_tri.cpu(), atol=0.01)
...
def test_dot_without_load(dtype_str, device):
    ...
    a = torch.ones((32, 32), dtype=getattr(torch, dtype_str)).to(device)
    b = torch.ones((32, 32), dtype=getattr(torch, dtype_str)).to(device)
    out_ref = torch.matmul(a.cpu(), b.cpu())
...
def test_arange(start, num_ctas, device):
    BLOCK = 128
    z_tri = torch.empty(BLOCK, dtype=torch.int32).to(device)
    ...
    np.testing.assert_allclose(to_numpy(z_tri.cpu()), to_numpy(z_ref))
...
def test_masked_load(dtype_str, size, size_diff, other, num_ctas, device):
    ...
    reference_out = torch.cat((input.cpu(), torch.full((size_diff, ), other, dtype=dtype)))
...
def test_vectorization(N, num_ctas, device):
    ...
    src = torch.randn(block_size).to(device)
    dst = torch.empty(block_size).to(device)
...
def test_vectorization_hints(has_hints, device):
    src = torch.empty(1024).to(device)
    dst = torch.empty(1024).to(device)
...
def test_value_specialization(value: int, value_type: str, device) -> None:
    ...
    x = torch.tensor([3.14159]).to(device)
    h = kernel.warmup(value, 1, x, grid=(1, ))
    assert "is_one" in h.name
    assert value_type in h.name


@pytest.mark.parametrize("value, overflow", [(2**64 - 1, False), (2**64, True), (-(2**63), False),
                                             (-(2**63) - 1, True)])
...
def test_bin_op_constexpr(op, is_lhs_constexpr, is_rhs_constexpr, device):
    ...
    x_tri = to_triton_thrive(x, device=device)
    y_tri = to_triton_thrive(y, device=device)
    z_tri = to_triton_thrive(np.empty((1, ), dtype=z.dtype), device=device)
...
def test_constexpr_shape(device):
    ...
    x_tri = to_triton_thrive(np.empty((256, ), dtype=np.int32), device=device)
    kernel[(1, )](x_tri)
...
def test_reshape(formats, device):
    ...
    if len(in_format) > 5 or len(out_format) > 5:
        pytest.skip(f"[Thrive] thnn don't support rank > 5 (in:{len(in_format)}, out:{len(out_format)})")
    ...
    z_tri = to_triton_thrive(np.empty(out_format, dtype=np.int32), device=device)
...
def test_tma_load_block_shape_err(device):
    ...
    input = torch.empty((128, 128), dtype=torch.int32).to(device)
...
def test_tma_store_block_shape_err(device):
    ...
    input = torch.empty((128, 128), dtype=torch.int16).to(device)
    errc = triton.CompilationError if not is_interpreter() else InterpreterError
    with pytest.raises(errc) as e:
        kernel[(1, )](input)
...
def test_trans_reshape(device, with_allocator):
    ...
    shape = (32, 32)
    input = torch.arange(math.prod(shape), dtype=torch.int32).reshape(shape).to(device)
    ...
    # no similar ir information in thrive
...
def test_call(type, num_ctas, device):
    ...
    rand_val_tri = to_triton_thrive(rand_val, device=device)
    err_msg = ""
    try:
...
def test_inline_asm(num_ctas, device):
    ...
    y_tri = to_triton_thrive(y, device=device)
    n = 17
    z_tri = to_triton_thrive(numpy_random(shape, dtype_str="uint32", rs=rs), device=device)
...
def test_inline_asm_packed(num_ctas, device):
    ...
    x_tri = to_triton_thrive(x, device=device)
    y_tri = to_triton_thrive(numpy_random(shape, dtype_str="uint8", rs=rs), device=device)
...
def test_inline_asm_multiple_outputs(device):
    ...
    A_tri = to_triton_thrive(A, device=device)
    B_tri = to_triton_thrive(B, device=device)
    C_tri = to_triton_thrive(numpy_random(shape, dtype_str="uint32", rs=rs), device=device)
    D_tri = to_triton_thrive(numpy_random(shape, dtype_str="uint32", rs=rs), device=device)
...
def test_inline_asm_packed_multiple_outputs(device):
    ...
    A_tri = to_triton_thrive(A, device=device)
    B_tri = to_triton_thrive(B, device=device)
    C_tri = to_triton_thrive(numpy_random(shape, dtype_str="int32", rs=rs), device=device)
    D_tri = to_triton_thrive(numpy_random(shape, dtype_str="float32", rs=rs), device=device)
    kernel[(1, )](A_tri, B_tri, C_tri, D_tri, BLOCK=shape[0])
...
def test_map_elementwise(num_ctas, device):
    ...
    x_tri = to_triton_thrive(x, device=device)
    y_tri = to_triton_thrive(y, device=device)
    z_tri = to_triton_thrive(numpy_random(shape, dtype_str="int32", rs=rs), device=device)
...
def test_for_iv(lo, hi, iv, device):
    ...
    lo = 2**35
    hi = 2**35 + 20
    out = to_triton_thrive(np.zeros((1, ), dtype=np.int64), device=device)
    kernel[(1, )](out, lo, hi, iv)
    assert out.cpu()[0] == sum(range(lo, hi, iv))
...
def test_if_else(device):
    ...
    out = to_triton_thrive(np.zeros((1, ), dtype=np.int32), device=device)
    true_val = to_triton_thrive(np.full((1, ), 1, dtype=np.int32), device=device)
    false_val = to_triton_thrive(np.full((1, ), 2, dtype=np.int32), device=device)
    cond = to_triton_thrive(np.zeros((1, ), dtype=np.int32), device=device)
    # True
    cond[0] = True
    kernel[(1, )](cond, true_val, false_val, out)
    assert to_numpy(out.cpu())[0] == true_val.cpu()[0]
    # False
    cond[0] = False
    kernel[(1, )](cond, true_val, false_val, out)
    assert to_numpy(out.cpu())[0] == false_val.cpu()[0]
...
def test_if_return(mode, device):
    ...
    out = to_triton_thrive(np.zeros((1, ), dtype=np.int32), device=device)
    exit_early = to_triton_thrive(np.zeros((1, ), dtype=np.int32), device=device)
...
def test_if_call(call_type, device):
    ...
    out = to_triton_thrive(np.zeros((1, ), dtype=np.int32), device=device)
    if device == "thrive":
        kernel[(1, )](out, call_type, num_cores=1)
    else:
        kernel[(1, )](out, call_type)
    assert to_numpy(out.cpu())[0] == 1
...
def test_nested_if_else_return(_cond1, _cond2, _cond3, device):
    ...
    out = to_triton_thrive(np.full((1, ), -1, dtype=np.int32), device=device)
    cond1 = to_triton_thrive(np.full((1, ), _cond1, dtype=np.int32), device=device)
    cond2 = to_triton_thrive(np.full((1, ), _cond2, dtype=np.int32), device=device)
    cond3 = to_triton_thrive(np.full((1, ), _cond3, dtype=np.int32), device=device)
    val1 = to_triton_thrive(np.full((1, ), 1, dtype=np.int32), device=device)
    val2 = to_triton_thrive(np.full((1, ), 2, dtype=np.int32), device=device)
    val3 = to_triton_thrive(np.full((1, ), 3, dtype=np.int32), device=device)
    kernel[(1, )](cond1, cond2, cond3, val1, val2, val3, out)
    targets = {
        (True, True, True): val1.cpu()[0],
        (True, True, False): val1.cpu()[0],
        (True, False, True): out.cpu()[0],
        (True, False, False): out.cpu()[0],
        (False, True, True): val2.cpu()[0],
        (False, True, False): val3.cpu()[0],
        (False, False, True): val2.cpu()[0],
        (False, False, False): val3.cpu()[0],
    }
    assert out.cpu()[0] == targets[(_cond1, _cond2, _cond3)]
...
def test_while(device):
    ...
    out_i = to_triton_thrive(np.zeros((1, ), dtype=np.int32), device=device)
    out_j = to_triton_thrive(np.zeros((1, ), dtype=np.int32), device=device)
    init_i = to_triton_thrive(np.full((1, ), 1, dtype=np.int32), device=device)
    out_init_i = to_triton_thrive(np.full((1, ), 0, dtype=np.int32), device=device)
    bound = to_triton_thrive(np.full((1, ), 10, dtype=np.int32), device=device)
    cut_off = to_triton_thrive(np.full((1, ), 5, dtype=np.int32), device=device)
    kernel[(1, )](init_i, bound, cut_off, out_i, out_init_i, out_j)
    assert out_init_i.cpu()[0] == init_i.cpu()[0]
    assert out_i.cpu()[0] == init_i.cpu()[0] + 1
    assert out_j.cpu()[0] == bound.cpu()[0]
...
def test_nested_while(device):
    ...
    counter = torch.tensor([8], dtype=torch.int32).to(device)
...
def test_constexpr_if_return(device):
    ...
    out = torch.empty((), dtype=torch.int32).to(device)
    kernel[(1, )](sem, out, 1)
...
def test_poison_return(device):
    ...
    a = torch.empty((), dtype=torch.int32).to(device)
...
def test_num_threads(device):
    ...
    num_threads = 256
    out = to_triton_thrive(np.zeros((num_threads, ), dtype=np.int32), device=device)
...
def test_globaltimer(device):
    ...
    out1 = to_triton_thrive(np.zeros((128, ), dtype=np.int64), device=device)
    out2 = to_triton_thrive(np.zeros((2, ), dtype=np.int64), device=device)
    if is_cuda():
...
def test_smid(device):
    ...
    out = to_triton_thrive(np.zeros((1024, ), dtype=np.int32), device=device)
...
def test_load_scalar_with_mask(device):
    ...
    Index = torch.tensor([0], dtype=torch.int32).to(device)
    Input = torch.tensor([0], dtype=torch.int32).to(device)
    Out = torch.empty_like(Index).to(device)
...
def test_ptx_cast(dtype_str, device):
    ...
    s0 = 4
    buf11 = -torch.ones((6 * s0, 197, 197), dtype=torch_dtype).to(device)
    buf14 = -torch.ones((s0, 6, 197, 197), dtype=torch_dtype).to(device)
...
def test_dot_max_num_imprecise_acc(M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, in_type_str, low_precision_acc, device):
    ...
    if is_cuda():
        ...
        if in_type_str not in ("float8e5", "float8e4nv"):
            pytest.skip(f"[Thrive] {in_type_str} not supported on thrive")

    check_type_supported(in_type_str, device)
    A = numpy_random((M, K), dtype_str=in_type_str)
    B = numpy_random((K, N), dtype_str=in_type_str)
    C = torch.empty((M, N), dtype=torch.float32).to(device)
    num_warps = 8
    a = to_triton_thrive(A, device=device, dst_type=in_type_str)
    b = to_triton_thrive(B, device=device, dst_type=in_type_str)
    ...
    torch_a = torch.from_numpy(A).to(device)
    th_a = f8_to_f16(torch_a, in_type_str)
    torch_b = torch.from_numpy(B).to(device)
    th_b = f8_to_f16(torch_b, in_type_str)
    ref_out = torch.matmul(th_a.cpu(), th_b.cpu()).to(torch.float32)
    if in_type_str == "float8e4nv":
        torch.testing.assert_close(ref_out, C.cpu(), rtol=0.01, atol=0.01)
    else:
        torch.testing.assert_close(ref_out, C.cpu(), rtol=1e-3, atol=1e-3)
...
def test_enable_reflect_ftz(enable_reflect_ftz, device, fresh_knobs):
    ...
    data = torch.full((128, ), -127.0, dtype=torch.float32).to(device)
...
def test_propagate_nan(dtype, propagate_nan, func, device):
    ...
    for mode in ["A", "B", "both"]:
        if func == "clamp" and mode == "B":
            ...
            continue
        A = torch.randn((1, ), dtype=getattr(torch, dtype)).to(device)
        if mode == "A" or mode == "both":
            A[0] = torch.nan
        B = torch.randn((1, ), dtype=getattr(torch, dtype)).to(device)
        if mode == "B" or mode == "both":
            B[0] = torch.nan
        C = torch.zeros_like(A, dtype=getattr(torch, dtype)).to(device)
...
def test_clamp(dtype, device):
    ...
    x = torch.randn((size, ), dtype=getattr(torch, dtype)).to(device)
    a = torch.randn((size, ), dtype=getattr(torch, dtype)).to(device)
    b = torch.randn((size, ), dtype=getattr(torch, dtype)).to(device)
    _min = torch.min(a, b)
    _max = torch.max(a, b)
    out = torch.zeros_like(x, dtype=getattr(torch, dtype)).to(device)
    ref = torch.zeros_like(x, dtype=getattr(torch, dtype)).to(device)
...
def test_clamp_symmetric(dtype, device):
    ...
    x = torch.randn((size, ), dtype=getattr(torch, dtype)).to(device)
    limit = torch.randn((size, ), dtype=getattr(torch, dtype)).abs().to(device)
    out = torch.zeros_like(x, dtype=getattr(torch, dtype)).to(device)
    ref = torch.zeros_like(x, dtype=getattr(torch, dtype)).to(device)
...
def test_static_range(device):
    ...
    Acc = torch.tensor([0], dtype=torch.int32).to(device)
    for i in range(0, N, step):
        Acc += i
    assert (Out.cpu() == Acc).all(), (Out.cpu(), Acc)
...
def test_tl_range_num_stages(device):
    ...
    ref_out = torch.matmul(a.cpu(), b.cpu()).to(torch.float32)
    if is_interpreter():
...
def test_tl_range_fuse_dependent(device):
    ...
    ub = 10
    out_i = torch.zeros(1024, dtype=torch.int32).to(device)
    ...
    assert thrir.count("scf.for") == 1

    ref_i = torch.zeros(1024, dtype=torch.int32).to(device)
    ref_j = torch.zeros(1024, dtype=torch.int32).to(device)
    k = 0
    for i in range(ub):
        lower_bound = i * 2
        upper_bound = lower_bound + i + 1
    ...
    torch.testing.assert_close(out_j.cpu(), ref_j.cpu(), atol=0, rtol=0)
...
def test_maxnreg(device):
    ...
    X = torch.empty(1, dtype=torch.int32).to(device)
...
def test_temp_var_in_loop(device):
    ...
    N = 10
    BLOCK = 32
    out = torch.empty((BLOCK, ), dtype=torch.int32).to(device)
...
def test_num_programs(device):
    ...
    grid = (11, 21, 31)
    input = torch.empty((3, ), dtype=torch.int32).to(device)
...
@pytest.mark.skip(reason="[Thrive] atomic_rmw on float32 not supported on thrive")
...
def test_gather(src_shape, indices_shape, axis, device):
    ...
    def triton_gather(src: torch.Tensor, axis: int, indices: torch.Tensor):
        output = torch.empty(indices.shape, dtype=src.dtype).to(src.device)
...
def test_jit_function_arg(device):
    ...
    out = torch.empty((BLOCK_SIZE, )).to(device)
...
def test_zero_strided_tensors(device):
    ...
    assert torch.allclose(x.cpu(), torch.ones_like(x.cpu()) * c_dim)
...
def test_aliasing(device):
    ...
    buffer = torch.zeros(1).to(device)
    aliasing_kernel[(1, )](buffer, buffer)
    assert buffer.cpu()[0] == 1
...
def test_strided_load(dtype, device):
    ...
    x_tri = to_triton_thrive(x, device)
    out_tri = torch.empty(OUT_SIZE).to(device)
    ...
    np.testing.assert_allclose(x[::2], to_numpy(out_tri.cpu()))
...
def test_indirect_load(dtype, device):
    ...
    ptr = torch.arange(SIZE, dtype=torch.int32).flip(0).to(device)
    out_tri = torch.empty(SIZE).to(device)
    indirect_load[(1, 1)](ptr, x_tri, out_tri, SIZE)

    np.testing.assert_allclose(np.flip(x), to_numpy(out_tri.cpu()))
...
def test_short_circuiting(device):
    ...
    def g(y, dtype):
        x = torch.full((1, ), y, dtype=dtype).to(device)
        f(x)
        return x.cpu().item()
...
def test_unsplat(device):
    ...
    def g(y, explicit):
        x = torch.full((1, ), y, dtype=torch.int32).to(device)
...
def test_cumsum_dtype(device):
    ...
    z = torch.zeros(4, dtype=torch.int32).to(device)
    kernel[(1, )](z)
    expected = torch.tensor([1, 2, 3, 4], dtype=torch.int32).to(device)
...
def test_dot_multidim(rank, trans_a, trans_b, device):
    ...
    a = torch.randint(-4, 5, shape, dtype=torch.bfloat16)
    b = torch.randint(-4, 5, shape, dtype=torch.bfloat16)
    c = torch.empty(shape, dtype=torch.float32).to(device)
    kernel[(1, )](a.to(device), b.to(device), c, rank, trans_a, trans_b)
