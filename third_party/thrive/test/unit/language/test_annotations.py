...
@pytest.mark.parametrize(("signed", "width"),
                         [(signed, width) for signed in [False, True] for width in [8, 16, 32, 64]] + [(False, 1)])
def test_int_annotation(signed, width, device):
    ...
    h = _kernel[(1, )](torch.empty(1).to(device), 3)
...
def test_float_annotation(device, dtype, test_val):
    ...
    ptr = torch.empty(1, dtype=torch.float32).to(device)
