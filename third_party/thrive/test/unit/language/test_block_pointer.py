...
from triton.backends.thrive.test.unit.language.test_core import check_type_supported
...
def test_block_copy(dtypes_str, n, padding_option, boundary_check, device):
    ...
    if src_dtype_str in ("bool", "int16", "int32"):
        ...
        a = torch.randint(0, 2, (n, ), dtype=src_dtype).to(device)
    else:
        a = torch.randn((n, ), dtype=src_dtype).to(device)
    b = torch.zeros((n, ), dtype=dst_dtype).to(device)
...
def test_block_ptr_matmul_no_scf(shape, num_warps, device):
    ...
    golden = torch.matmul(a.cpu(), b.cpu())
