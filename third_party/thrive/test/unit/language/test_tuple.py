...
def test_index(size, device):
    ...
    rets = tuple([torch.zeros((1, ), dtype=torch.float32).to(device) for _ in vals])
...
def test_assign_return(device):
    ...
    x = torch.tensor([1.3], dtype=torch.float32).to(device)
    y = torch.tensor([1.9], dtype=torch.float32).to(device)
    a_tri = torch.tensor([0], dtype=torch.float32).to(device)
    b_tri = torch.tensor([0], dtype=torch.float32).to(device)
    c_tri = torch.tensor([0], dtype=torch.float32).to(device)
...
def test_serialize(device):
    x0 = torch.tensor([8], dtype=torch.int32).to(device)
    x1 = torch.tensor([12], dtype=torch.int32).to(device)
    y0 = torch.tensor([10], dtype=torch.int32).to(device)
    z = torch.empty((10, ), dtype=torch.int32).to(device)
    ...
    ref = torch.tensor([8, 1, 12, 21, 10, 15, -1, 8, 1, 12]).to(device)
...
def test_namedtuple(device):
    x = torch.randn((32, 32), dtype=torch.float32).to(device)
    y = torch.empty((16, 16), dtype=torch.float32).to(device)
    a = torch.tensor([5.2], dtype=torch.float32).to(device)
...
def test_eq(device):
    ...
    rets = torch.zeros((4, ), dtype=torch.int32).to(device)
    fn[(1, )](rets)
...
def test_add(device):
    ...
    rets = torch.zeros((8, ), dtype=torch.int32).to(device)
    fn[(1, )](rets)
...
def test_passing_tuple_with_constexpr(device):
    ...
    x = torch.arange(0, 64).reshape(8, 8)
    expected_x = 8 * x.clone()
    m_to_the_n[(1, )](x.to(device), x.shape, x.stride(), (2, 3))
...
def test_passing_tuple_to_make_tensor_descriptor(device, with_allocator):
    ...
    expected_x = 8 * x.clone()
    m_to_the_n[(1, )](x.to(device), x.size(), x.stride(), (2, 3), x.size(0))
    torch.testing.assert_close(x, expected_x, rtol=0, atol=0)
