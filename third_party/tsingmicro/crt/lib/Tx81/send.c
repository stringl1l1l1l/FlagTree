...
void tile_sync_by_spm_single_direction(int32_t tile_this, int32_t tile_a, int32_t tile_x, int32_t tile_y,
    ...
    volatile uint32_t *this_debug_spm_ptr = (volatile uint32_t *)get_spm_memory_mapping(SINGLE_SPM_SYNC_DEBUG_ADDR);
    ...
    uint64_t tile_a_spm = get_tile_spm_addr_base(tile_a, tile_x, tile_y) + SINGLE_SPM_SYNC_ADDR;
    *(uint64_t *)(this_debug_spm_ptr + 6) = tile_a_spm;
    volatile uint32_t *this_spm_ptr = (volatile uint32_t *)get_spm_memory_mapping(SINGLE_SPM_SYNC_ADDR);
...
int initTileId(uint32_t tileId, uint32_t rowLength) {
    ...
    *(volatile uint32_t *)(get_spm_memory_mapping(TILE_ID_ADDR)) = tileId;
    // init 2D logic id
    *(volatile uint32_t *)(get_spm_memory_mapping(LOGIC_ID_ADDR)) = *(volatile uint32_t *)(SCFG_TILE_ID_ADDR);
    // init row-length
    ...
    *(volatile uint32_t *)(get_spm_memory_mapping(INNER_CHIP_ERROR_CODE)) = 0;
    ...
    //         *(volatile uint32_t *)(get_spm_memory_mapping(LOGIC_ID_ADDR)), tileId, rowLength);
    return 0;
}
...
void __Send(int64_t chipX, int64_t chipY, int64_t dieId, int64_t tileId,
            void *restrict dst, void *restrict src,
