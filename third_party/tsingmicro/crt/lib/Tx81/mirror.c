...
//===----------------------------------------------------------------------===//
//
// Runtime API of MLIR operation tx::Mirror see Tx81Ops.td for detail.
//
//===----------------------------------------------------------------------===//

#include "tx81_run.h"

void __Mirror(uint64_t *src, uint16_t src_n, uint16_t src_h, uint16_t src_w,
  ...
  cmd->Mirror(&inst, (uint64_t)src, shape1, (uint64_t)dst, shape2,
              (Data_Format)fmt);
