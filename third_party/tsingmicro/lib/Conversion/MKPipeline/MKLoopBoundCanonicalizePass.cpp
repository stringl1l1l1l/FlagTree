...
Value getLoopInductionValue(Value value) {
    if (auto cast = value.getDefiningOp<arith::IndexCastOp>()) return cast.getIn();
    return value;
}

bool sameValue(Value lhs, Value rhs) { return getLoopInductionValue(lhs) == getLoopInductionValue(rhs); }

Value createIntegerConstant(PatternRewriter &rewriter, Location loc, Type type, int64_t value) {
    if (type.isIndex()) return rewriter.create<arith::ConstantIndexOp>(loc, value);
    return rewriter.create<arith::ConstantOp>(loc, type, rewriter.getIntegerAttr(type, value));
}

bool matchAddOfIvAndStep(arith::AddIOp addOp, Value ivLike, int64_t &stepValue) {
...
bool matchMinOfAddAndBound(arith::MinSIOp minOp, arith::AddIOp &addOp, int64_t &boundValue) {
...
bool matchMaxOfMinAndIv(arith::MaxSIOp maxOp, Value ivLike, arith::MinSIOp &minOp) {
...
bool loopHasOnlyFullTiles(scf::ForOp forOp, int64_t clampUpper, int64_t tileStep) {
    ...
    if (!lower || !upper || !loopStep) return false;

    if (*loopStep <= 0 || tileStep <= 0) return false;
    ...
    if (tileStep != *loopStep || clampUpper != *upper) return false;
...
bool isTailCmp(arith::CmpIOp cmpOp, Value size, int64_t tileStep) {
    if (cmpOp.getPredicate() != arith::CmpIPredicate::slt) return false;
    ...
    return (lhs == size && matchConstStep(rhs)) || (rhs == size && matchConstStep(lhs));
}
...
struct FullTileTailSizePattern : OpRewritePattern<arith::SubIOp> {
    ...
    LogicalResult matchAndRewrite(arith::SubIOp subOp, PatternRewriter &rewriter) const override {
        auto forOp = subOp->getParentOfType<scf::ForOp>();
        if (!forOp) return failure();

        Value ivLike = subOp.getRhs();
        if (getLoopInductionValue(ivLike) != forOp.getInductionVar()) return failure();

        auto maxOp = subOp.getLhs().getDefiningOp<arith::MaxSIOp>();
        if (!maxOp) return failure();

        arith::MinSIOp minOp;
        if (!matchMaxOfMinAndIv(maxOp, ivLike, minOp)) return failure();

        arith::AddIOp addOp;
        int64_t clampUpper = 0;
        if (!matchMinOfAddAndBound(minOp, addOp, clampUpper)) return failure();

        int64_t tileStep = 0;
        if (!matchAddOfIvAndStep(addOp, ivLike, tileStep)) return failure();

        if (!loopHasOnlyFullTiles(forOp, clampUpper, tileStep)) return failure();

        SmallVector<arith::CmpIOp> tailCmps;
        for (Operation *user : llvm::make_early_inc_range(subOp->getUsers())) {
            if (auto cmpOp = dyn_cast<arith::CmpIOp>(user); cmpOp && isTailCmp(cmpOp, subOp.getResult(), tileStep))
                tailCmps.push_back(cmpOp);
        }

        for (arith::CmpIOp cmpOp : tailCmps) {
            rewriter.setInsertionPoint(cmpOp);
            Value falseValue = rewriter.create<arith::ConstantOp>(cmpOp.getLoc(), rewriter.getBoolAttr(false));
            rewriter.replaceOp(cmpOp, falseValue);
        }

        rewriter.setInsertionPoint(subOp);
        Value fullTileSize = createIntegerConstant(rewriter, subOp.getLoc(), subOp.getType(), tileStep);
...
struct MKLoopBoundCanonicalizePass : public triton::impl::MKLoopBoundCanonicalizePassBase<MKLoopBoundCanonicalizePass> {
