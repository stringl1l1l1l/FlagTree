...
def check_type_supported(dtype, device):
    ...
    # TODO
    if dtype in ["float64"]:
        pytest.skip("Not supported on gcu: dtype fp64 is not supported on gcu")
    elif dtype in ["tf32"]:
        pytest.skip("Not supported on gcu: dtype tf32 is not supported on gcu")
    elif dtype in [tl.float8e4nv, "float8e4nv"] and target_arch == 'gcu300':
...
def test_tensor_atomic_add_access_patterns(shape, idx_order, mask_step, num_ctas, dtype_x_str, device):
    ...
    shape0, shape1 = shape
    idx_row = torch.arange(0, shape1, device=device, dtype=torch.int32)  # TODO
    ...
    if idx_order == 'random_no_duplication':
        idx = torch.stack([torch.randperm(shape1, device=device, dtype=torch.int32) for _ in idx_row])  # TODO
    if idx_order == 'random':
        idx = torch.randint(0, shape1, size=(shape0, shape1), device=device, dtype=torch.int32)  # TODO
...
def test_cat(dtype_str, num_warps, device):
    ...
    # TODO(chongzhou.yang): fallback it to cpu until topsatenunique2 is supported
    assert z.to('cpu').unique().size(0) == z.to('cpu').size(0)
...
def test_split_to_scalar(device):
    ...
    N = 2
    # TODO: not support int64
    # x = torch.arange(0, N, device=device).reshape(N // 2, 2)
    x = torch.arange(0, N, device=device).to(torch.int32).reshape(N // 2, 2)
    z1_ref, z2_ref = (x[:, 0], x[:, 1])
    z1 = torch.zeros_like(z1_ref)
    z2 = torch.zeros_like(z2_ref)
    kernel[(1, )](x, z1, z2)
...
def test_max_min_with_nan(device):
    ...
    # in the reduce dimension, we should ignore it and return the max/min number,
...
def test_scan2d(op, dtype_str, shape, axis, reverse, num_warps, device):
    ...
    if op == 'cumsum' or op == 'cumprod':
        ...
        ## TODO: not support 64
        z = np.empty_like(x, dtype=np.int32)
...
def test_generic_reduction(device):
    ...
    # TODO(chongzhou.yang): torch.var_mean is not yet supported in gcu400, fallback it to cpu until 2025.12.31
    expect_var, expect_mean = torch.var_mean(x.to('cpu'), dim=0, correction=0)
    sum_ref = torch.sum(x)
    torch.testing.assert_close(out_mean, expect_mean.to('gcu'))
    torch.testing.assert_close(out_var, expect_var.to('gcu'))
...
def test_trans_4d(dtype_str, shape, perm, device, with_allocator):
    ...
    # TODO
    # input = torch.arange(math.prod(shape), dtype=getattr(torch, dtype_str), device=device).reshape(shape)
    input = torch.arange(math.prod(shape), device=device).to(getattr(torch, dtype_str)).reshape(shape)
    expected = torch.permute(input, perm)
    # Don't do zeros_like -- that copies the layout, which we don't want.
    actual = torch.zeros(expected.shape, dtype=getattr(torch, dtype_str), device=device)

    # num_warps in gcu4xx can not exceed 4
    kernel[(1, )](input, actual, *shape, *[shape[i] for i in perm], *perm, num_warps=4)
...
def test_dot(M, N, K, num_warps, col_a, col_b, epilogue, input_precision, in_dtype, out_dtype, kpack, mma_nonk_size,
    ...
    if is_interpreter():
        ...
        if not is_hip() and (M < 16 or N < 16 or K < 16):  # TODO
        ...
        if device == 'gcu':
            if in_dtype == 'float64' or out_dtype == 'float64':
                pytest.skip("float64 not supported on gcu yet")
    ...
    # TODO Workaround: gcu400 does not support fp64 type
...
def matmul_kernel(  #
    ...
    pid_m = pid % num_pid_m
    pid_n = pid // num_pid_m
    # TODO: http://jira.enflame.cn/browse/TCC-2285
    # offs_am = (pid_m * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)) % M
    # offs_bn = (pid_n * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)) % N
    offs_am = pid_m * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)
    mask_a = (offs_am < M)[:, None]
    offs_bn = pid_n * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)
    mask_b = (offs_bn < N)[None, :]
...
def test_gather(src_shape, indices_shape, axis, device):
    ...
    # TODO
    indices = torch.randint(0, src.shape[axis], indices_shape, device=device, dtype=torch.int32)  # TODO
...
def test_dot_multidim(rank, trans_a, trans_b, device):
    ...
    def kernel(X, Y, Z, RANK: tl.constexpr, TRANS_A: tl.constexpr, TRANS_B: tl.constexpr):
        x = tl.load(X + tl.arange(0, 256 << RANK)).reshape([2] * (RANK - 2) + [32, 32])  # type: ignore[operator]
        y = tl.load(Y + tl.arange(0, 256 << RANK)).reshape([2] * (RANK - 2) + [32, 32])  # type: ignore[operator]
        if TRANS_A:
            x = tl.trans(x)
        if TRANS_B:
            y = tl.trans(y)
        z = tl.dot(x, y)
        tl.store(Z + tl.arange(0, 256 << RANK), z.reshape([256 << RANK]))  # type: ignore[operator]
