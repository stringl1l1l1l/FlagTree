...
// triton_gcu300_core.cpp -- In-process replacement for triton-gcu300-opt.
//
// Registers the same GCU dialects/passes as the standalone executable,
// then runs the requested pass pipeline on MLIR text in-process.
// Compiles into lib_triton_gcu300_core.so (zero Python dependency).
...
// Global pass registration -- idempotent, guarded by call_once.
static std::once_flag g_passRegFlag;
static void ensurePassesRegistered() {
  std::call_once(g_passRegFlag, []() {
    mlir::DialectRegistry dummy;
    mlir::gcu::registerGCUDialects(dummy);
    mlir::triton::gpu::registerTritonGPUPasses();
    mlir::triton::registerConvertTritonToTritonGPUPass();
  });
}

static Gcu300String makeString(const std::string &s) {
...
static Gcu300String nullString() { return {nullptr, 0}; }

// ===== Pipeline Implementation =====

struct Gcu300Pipeline_ {
  ...
  Gcu300Pipeline_() { ensurePassesRegistered(); }
};
...
Gcu300Pipeline gcu300_pipeline_create(void) {
  try {
    return new Gcu300Pipeline_();
  } catch (const std::exception &e) {
    llvm::errs() << "[gcu300_pipeline_create] exception: " << e.what() << "\n";
    return nullptr;
  }
}
  ...
  for (const auto &[name, opts] : p->passes) {
    ...
    if (spec.find('(') != std::string::npos) {
      if (mlir::failed(mlir::parsePassPipeline(spec, pm))) {
        ...
        llvm::errs() << "[gcu300_pipeline_run] " << p->lastError << "\n";
        return nullString();
      }
      continue;
    }
...
static Gcu300String runOptOnChunk(llvm::StringRef chunk,
  ...
  for (const auto &arg : passArgs) {
    ...
    if (normalized.find('(') != std::string::npos) {
      if (mlir::failed(mlir::parsePassPipeline(normalized, pm))) {
        llvm::errs() << "[gcu300_run_opt] failed to parse pass pipeline: "
                     << normalized << "\n";
        return nullString();
      }
      continue;
...
Gcu300String gcu300_run_opt(const char *input, size_t input_len,
  ...
  for (int i = 0; i < num_args; ++i) {
    ...
    if (a.starts_with("-mlir-print-ir-before"))
      continue;
    // Strip leading dashes: "-pass-name" / "--pass-name" -> "pass-name"
  ...
  // Split at "// -----" markers (standard MLIR split-input-file delimiter)
